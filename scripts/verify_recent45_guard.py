...
def evaluate(label, config, watch=None):
    ...
    print(f"\n=== {label} ===")
    print(f"  recent-45 (incl 07-31): {mae(prod_final, pd.Series(True, index=df.index)):.4f} -> {mae(new_final, pd.Series(True, index=df.index)):.4f}")
    print(f"  recent-45 (excl 07-31): {mae(prod_final, EXCL):.4f} -> {mae(new_final, EXCL):.4f}")
    print(f"  rows newly touched     : {int((incr.abs() > 1e-6).sum())}")
    for dd in (watch or []):
        mm = df["Date"] == dd
        if mm.any():
            print(f"    {dd}: {mae(prod_final, mm):.2f} -> {mae(new_final, mm):.2f} (n={int(mm.sum())})")
    return new_final


if __name__ == "__main__":
    watch = ["2026-07-20", "2026-07-21", "2026-07-19", "2026-07-17", "2026-07-09",
             "2026-07-04", "2026-06-23", "2026-07-10", "2026-07-30", "2026-07-16",
             "2026-07-24", "2026-07-26", "2026-07-28", "2026-07-29", "2026-07-27"]
    ...
    edit_rule(c1, "july_recent_95_99_partly_cloudy_high_state_peak_lift", min_cloud_cover_norm=0.0)
    ...
    edit_rule(c2, "july_recent_95_99_partly_cloudy_high_state_peak_lift",
              min_cloud_cover_norm=0.0, hours=[12, 13, 14, 15, 16, 17, 18])
    ...
    edit_rule(c3, "july_recent_95_99_partly_cloudy_high_state_peak_lift", min_cloud_cover_norm=0.0)
    add_rule_after(c3, "july_recent_95_99_partly_cloudy_high_state_peak_lift", {
    ...
    add_rule_after(cf, "july_recent_95_99_partly_cloudy_high_state_peak_lift", {
    ...
    add_rule_after(cf, "july_recent_95_99_clear_high_state_peak_lift", {
