...
def monotonic_constraints_enabled(config: dict | None) -> bool:
    return _as_bool(
        _cfg(config, "model", "monotonic_constraints", "enabled", default=True),
        default=True,
    )


def build_monotone_vector(
    features: list[str], config: dict | None
) -> tuple[int, ...] | None:
    ...
    increasing = set(
        mc_cfg.get("increasing_features", DEFAULT_INCREASING_FEATURES) or []
    )
    decreasing = set(
        mc_cfg.get("decreasing_features", DEFAULT_DECREASING_FEATURES) or []
    )
    ...
    vector = tuple(
        1 if f in increasing else (-1 if f in decreasing else 0) for f in features
    )
