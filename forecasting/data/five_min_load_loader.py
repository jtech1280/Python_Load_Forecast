...
def load_five_min_system_load(config: dict) -> pd.DataFrame:
    ...
    try:
        ...
        warnings.warn(f"5-minute load query failed; continuing without intraday load features. Details: {exc}", RuntimeWarning)
        return pd.DataFrame()

    if raw.empty or "StartPST" not in raw.columns or "Quantity" not in raw.columns:
        warnings.warn("5-minute load query returned no usable StartPST/Quantity columns.", RuntimeWarning)
        return pd.DataFrame()
