...
def load_local_station_weather(config: dict) -> pd.DataFrame:
    ...
    try:
        ...
        warnings.warn(f"Local weather query failed; continuing with Open-Meteo only. Details: {exc}", RuntimeWarning)
        return pd.DataFrame()

    required = {"DT", "READING"}
    if raw.empty or not required.issubset(raw.columns):
        warnings.warn("Local weather query returned no usable DT/READING columns.", RuntimeWarning)
        return pd.DataFrame()
    ...
    out.loc[(out["LocalStation_TempF"] < min_f) | (out["LocalStation_TempF"] > max_f), "LocalStation_TempF"] = np.nan
...
def build_temperature_bias_lookup(openmeteo_hist: pd.DataFrame, local_weather: pd.DataFrame, config: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if openmeteo_hist is None or openmeteo_hist.empty or local_weather is None or local_weather.empty:
    ...
    merged["LocalStation_TempF"] = pd.to_numeric(merged["LocalStation_TempF"], errors="coerce")
    merged["LocalStation_TempDelta_F"] = merged["LocalStation_TempF"] - merged["OpenMeteo_TempF"]
    merged = merged.dropna(subset=["DT", "OpenMeteo_TempF", "LocalStation_TempF", "LocalStation_TempDelta_F"]).copy()
    merged = merged[merged["LocalStation_TempDelta_F"].abs().le(max_abs_delta)].copy()
    if merged.empty:
        return pd.DataFrame(), pd.DataFrame()

    local_dt = pd.to_datetime(merged["DT"], utc=True).dt.tz_convert(config["project"]["timezone"])
    merged["Month"] = local_dt.dt.month
    merged["Hour"] = local_dt.dt.hour

    global_delta = float(merged["LocalStation_TempDelta_F"].mean())
    by_month = merged.groupby("Month")["LocalStation_TempDelta_F"].agg(["count", "mean"]).reset_index()
    by_month = by_month.rename(columns={"count": "Month_Count", "mean": "Month_Delta_F"})
    by_month_hour = merged.groupby(["Month", "Hour"])["LocalStation_TempDelta_F"].agg(["count", "mean", "std"]).reset_index()
    by_month_hour = by_month_hour.rename(columns={"count": "N", "mean": "Bias_Delta_F", "std": "Bias_Std_F"})
    ...
    lookup["Applied_Delta_F"] = pd.to_numeric(lookup["Applied_Delta_F"], errors="coerce").fillna(global_delta)
    return lookup, merged


def apply_temperature_bias_calibration(weather_df: pd.DataFrame, lookup: pd.DataFrame, config: dict) -> pd.DataFrame:
    cfg = ((config.get("local_weather", {}) or {}).get("temperature_calibration", {}) or {})
    if weather_df is None or weather_df.empty or lookup is None or lookup.empty or "TempF" not in weather_df.columns:
        return weather_df

    blend = float(cfg.get("blend", 1.0))
    cap_f = float(cfg.get("cap_f", 8.0))
    out = weather_df.copy()
    local_dt = pd.to_datetime(out["DT"], errors="coerce", utc=True).dt.tz_convert(config["project"]["timezone"])
    ...
    global_delta = float(pd.to_numeric(lookup["Global_Delta_F"], errors="coerce").dropna().iloc[0]) if "Global_Delta_F" in lookup.columns and pd.to_numeric(lookup["Global_Delta_F"], errors="coerce").notna().any() else 0.0
    out["TempF_OpenMeteo"] = pd.to_numeric(out["TempF"], errors="coerce")
    out["LocalStation_TempBias_F"] = pd.to_numeric(out["Applied_Delta_F"], errors="coerce").fillna(global_delta).clip(-cap_f, cap_f) * blend
...
def merge_local_station_temperature(weather_df: pd.DataFrame, local_weather: pd.DataFrame) -> pd.DataFrame:
    if weather_df is None or weather_df.empty or local_weather is None or local_weather.empty:
        return weather_df
    out = weather_df.copy()
    ...
    out = out.merge(local[["__DT_KEY", "LocalStation_TempF"]], on="__DT_KEY", how="left")
    out["LocalStation_TempDelta_F"] = pd.to_numeric(out["LocalStation_TempF"], errors="coerce") - pd.to_numeric(out.get("TempF_OpenMeteo", out.get("TempF")), errors="coerce")
    return out.drop(columns=["__DT_KEY"], errors="ignore")


def local_temperature_bias_summary(matched: pd.DataFrame, lookup: pd.DataFrame) -> pd.DataFrame:
    ...
    by_month = matched.groupby("Month")["LocalStation_TempDelta_F"].agg(
        N="count",
        Mean_Delta_F="mean",
        Median_Delta_F="median",
    ).reset_index()
    by_month["Segment"] = "Month " + by_month["Month"].astype(str)
    by_month["MAE_Delta_F"] = matched.groupby("Month")["LocalStation_TempDelta_F"].apply(lambda s: s.abs().mean()).to_numpy()
    by_month["P90_Abs_Delta_F"] = matched.groupby("Month")["LocalStation_TempDelta_F"].apply(lambda s: s.abs().quantile(0.90)).to_numpy()
    summary = pd.concat([pd.DataFrame(rows), by_month[["Segment", "N", "Mean_Delta_F", "Median_Delta_F", "MAE_Delta_F", "P90_Abs_Delta_F"]]], ignore_index=True)
