...
def _prep_context(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "IsWeekend" not in out.columns:
        ...
        out["IsWeekend"] = (
            pd.to_numeric(out["IsWeekend"], errors="coerce").fillna(0).astype(int)
        )
    ...
    solar = pd.to_numeric(
        out.get("BTM_Solar_Proxy_MW", pd.Series(0.0, index=out.index)), errors="coerce"
    ).fillna(0.0)
    out["BTMSolarProxyBin"] = pd.cut(
        solar,
        bins=[-0.001, 1, 5, 10, 15, 20, 30, 9999],
        labels=False,
        include_lowest=True,
    ).astype(float)
...
def _residual_work(backtest_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Residual_MWH" in work.columns:
        ...
        work["Residual"] = pd.to_numeric(
            work["Actual_MWH"], errors="coerce"
        ) - pd.to_numeric(work["Raw_Forecast_MWH"], errors="coerce")
...
def _hot_peak_season_scale(out: pd.DataFrame, hot_peak_cfg: dict | None) -> pd.Series:
    cfg = hot_peak_cfg or {}
    scale = pd.Series(
        float(cfg.get("default_scale", 1.0)), index=out.index, dtype=float
    )
    ...
    hot_peak = out["Hour"].isin(hours) & pd.to_numeric(
        out["Temperature_DailyMax"], errors="coerce"
    ).ge(float(cfg.get("min_maxtemp_f", 90.0)))
    for season, season_scale in season_scales.items():
        scale.loc[hot_peak & out["Season"].astype(str).eq(str(season))] = float(
            season_scale
        )
    return scale
...
def build_learned_residual_lookups(
    backtest_df: pd.DataFrame, blend: float, cap_mwh: float, shrink_k: float
) -> dict:
    ...
    return {
        ...
        "ordered_levels": [
            {
                "name": name,
                "keys": keys,
                "lookup": _make_lookup(work, keys, blend, cap_mwh, shrink_k),
            }
        ...
        "metadata": {
            ...
            "rmse": (
                float(np.sqrt(np.mean(np.square(work["Residual"]))))
                if len(work)
                else np.nan
            ),
...
def apply_learned_calibration(
    ...
    for level in lookup_bundle["ordered_levels"]:
        ...
        if keys:
            tmp = (
                out[keys]
                .reset_index()
                .merge(lookup[keys + ["correction", "count"]], on=keys, how="left")
            )
        else:
            # Global lookup: same correction applies to every row.
            correction = (
                float(lookup["correction"].iloc[0]) if "correction" in lookup else 0.0
            )
            count = float(lookup["count"].iloc[0]) if "count" in lookup else np.nan
            tmp = pd.DataFrame(
                {"index": out.index, "correction": correction, "count": count}
            )
        ...
        corr = (
            pd.to_numeric(tmp.loc[matched, "correction"], errors="coerce")
            .fillna(0.0)
            .to_numpy(dtype=float)
        )
        # Let higher-count lookups have a bit more influence without letting them dominate.
        cnt = (
            pd.to_numeric(tmp.loc[matched, "count"], errors="coerce")
            .fillna(0.0)
            .clip(lower=0.0)
            .to_numpy(dtype=float)
        )
    ...
    mask = weight_sum > 0
    out.loc[mask, "Residual_Cal_MWH"] = (
        weighted_sum.loc[mask] / weight_sum.loc[mask]
    ).astype(float)
    if cap_mwh is not None:
        out["Residual_Cal_MWH"] = out["Residual_Cal_MWH"].clip(
            -float(cap_mwh), float(cap_mwh)
        )
    out["Residual_Cal_MWH"] = out["Residual_Cal_MWH"] * _hot_peak_season_scale(
        out, hot_peak_cfg
    )
    out.loc[mask, "Calibration_Level"] = "weighted_fallback"
    out["Calibration_Matched_Levels"] = [
        "+".join(matched_names[idx]) for idx in out.index
    ]
    out["Residual_Calibrated_Forecast_MWH"] = (
        out[base_col].astype(float) + out["Residual_Cal_MWH"].astype(float)
    ).clip(lower=0.0)
    out["Calibrated_Forecast_MWH"] = out["Residual_Calibrated_Forecast_MWH"]
    return out


def build_heat_peak_lookup(
    ...
    if "Temperature_DailyMax" not in work.columns:
        return None
    mask = (
        pd.to_numeric(work["Temperature_DailyMax"], errors="coerce")
        >= float(min_maxtemp_f)
...
def apply_heat_peak_calibration(
    future_df: pd.DataFrame,
    heat_lookup: pd.DataFrame | None,
    min_maxtemp_f: float,
    hours: list[int],
) -> pd.DataFrame:
    out = _prep_context(future_df)
    out["Heat_Peak_Cal_MWH"] = 0.0
    out["Heat_Adjusted_Forecast_MWH"] = out.get(
        "Calibrated_Forecast_MWH", out["Raw_Forecast_MWH"]
    ).astype(float)
    if (
        heat_lookup is None
        or heat_lookup.empty
        or "Temperature_DailyMax" not in out.columns
    ):
    ...
    mask = (
        ...
        >= float(min_maxtemp_f)
    ) & (out["Hour"].isin([int(h) for h in hours]))
    tmp = (
        out.loc[mask, ["Hour", "DailyMaxTempBin"]]
        .reset_index()
        .merge(
            heat_lookup[["Hour", "DailyMaxTempBin", "correction"]],
            on=["Hour", "DailyMaxTempBin"],
            how="left",
        )
    )
    matched = tmp["correction"].notna()
    if matched.any():
        idx = tmp.loc[matched, "index"]
        out.loc[idx, "Heat_Peak_Cal_MWH"] = tmp.loc[matched, "correction"].to_numpy(
            dtype=float
        )
        out.loc[idx, "Calibration_Level"] = (
            out.loc[idx, "Calibration_Level"].astype(str) + "+heat_peak"
        )
    out["Heat_Adjusted_Forecast_MWH"] = (
        out.get("Calibrated_Forecast_MWH", out["Raw_Forecast_MWH"]).astype(float)
        + out["Heat_Peak_Cal_MWH"].astype(float)
    ).clip(lower=0.0)
    out["Calibrated_Forecast_MWH"] = out["Heat_Adjusted_Forecast_MWH"]
    return out
...
def build_warm_ramp_lookup(
    ...
    specs = [
        (
            "warm_season_hour_temp_cloud",
            ["Season", "Hour", "DailyMaxTempBin", "CloudCoverBin"],
        ),
...
def apply_warm_ramp_correction(
    ...
    out["Warm_Ramp_Adjusted_Forecast_MWH"] = out.get(
        "Calibrated_Forecast_MWH", out["Raw_Forecast_MWH"]
    ).astype(float)
    out["Warm_Ramp_Correction_Source"] = "none"
    if (
        not warm_lookup
        or not warm_lookup.get("ordered_levels")
        or "Temperature_DailyMax" not in out.columns
    ):
    ...
    out.loc[mask, "Warm_Ramp_Cal_MWH"] = (
        weighted_sum.loc[mask] / weight_sum.loc[mask]
    ).clip(-float(cap_mwh), float(cap_mwh))
    out.loc[mask, "Warm_Ramp_Correction_Source"] = [
        "+".join(matched_names[idx]) for idx in out.loc[mask].index
    ]
    out["Warm_Ramp_Adjusted_Forecast_MWH"] = (
        out.get("Calibrated_Forecast_MWH", out["Raw_Forecast_MWH"]).astype(float)
        + out["Warm_Ramp_Cal_MWH"].astype(float)
    ).clip(lower=0.0)
    out["Calibrated_Forecast_MWH"] = out["Warm_Ramp_Adjusted_Forecast_MWH"]
    return out
...
def build_learned_seasonal_lookup(
    backtest_df: pd.DataFrame, blend: float, cap_mwh: float, shrink_k: float
):
...
def apply_seasonal_calibration(
    future_df: pd.DataFrame, lookup: pd.DataFrame
) -> pd.DataFrame:
    if lookup is None or lookup.empty:
        future_df["Seasonal_Cal_MWH"] = 0.0
        future_df["Calibrated_Forecast_MWH"] = (
            future_df["Raw_Forecast_MWH"].astype(float).clip(lower=0.0)
        )
        return future_df
    out = future_df.merge(lookup, on=["Season", "Hour"], how="left")
    out["Seasonal_Cal_MWH"] = out["correction"].fillna(0.0)
    out["Calibrated_Forecast_MWH"] = (
        out["Raw_Forecast_MWH"].astype(float) + out["Seasonal_Cal_MWH"]
    ).clip(lower=0.0)
    return out
