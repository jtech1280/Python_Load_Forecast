...
from forecasting.data.history_loader import (
    actuals_import_cutoff_dt,
    load_hourly_system_mwh,
)
...
from forecasting.features.feature_builder import (
...
from forecasting.forecast.event_shape_corrections import (
    build_cloud_solar_shape_lookup,
    apply_cloud_solar_shape_correction,
)
...
def _trim_incomplete_future_weather(
    future_frame: pd.DataFrame, required_cols: list[str] | None = None
) -> pd.DataFrame:
    ...
    print(
        f"WARNING: Dropping {dropped} incomplete future weather rows starting at {first_bad_dt}."
    )
...
def _filter_to_actuals_cutoff(
    ...
    *,
    ...
    cutoff_utc = cutoff.tz_convert("UTC") + pd.Timedelta(
...
def build_correction_artifacts(
    ...
    *,
    ...
    if bool(targeted_meta_cfg.get("enabled", True)):
        artifacts["targeted_meta_artifact"] = build_targeted_residual_meta_model(
            raw_backtest_df, config
        )
        if artifacts["targeted_meta_artifact"]:
            ...
            residual_basis_df["Residual_MWH"] = pd.to_numeric(
                residual_basis_df["Actual_MWH"], errors="coerce"
            ) - pd.to_numeric(
                residual_basis_df["Targeted_Meta_Adjusted_Forecast_MWH"],
                errors="coerce",
            )
    ...
    if bool(cal_cfg.get("cloud_solar_shape_enabled", True)):
        ...
        pre_cloud_lookup_frame["Pre_Cloud_Residual_MWH"] = pd.to_numeric(
            pre_cloud_lookup_frame["Actual_MWH"], errors="coerce"
        ) - pd.to_numeric(
            pre_cloud_lookup_frame["Calibrated_Forecast_MWH"], errors="coerce"
        )
        artifacts["cloud_solar_lookup"] = build_cloud_solar_shape_lookup(
            backtest_df=pre_cloud_lookup_frame,
            hours=list(
                cal_cfg.get("cloud_solar_shape_hours", [10, 11, 12, 13, 14, 15, 16])
            ),
    ...
    if bool((cal_cfg.get("recent_residual", {}) or {}).get("enabled", True)):
        ...
        if recent_basis_col in recent_profile_frame.columns:
            recent_profile_frame["Residual_MWH"] = pd.to_numeric(
                recent_profile_frame["Actual_MWH"], errors="coerce"
            ) - pd.to_numeric(recent_profile_frame[recent_basis_col], errors="coerce")
        artifacts["recent_profile"] = build_recent_residual_profile(
            recent_profile_frame, config=config
        )
    if bool(
        (
            (
                (cal_cfg.get("stage_selector", {}) or {}).get(
        ...
        artifacts["focused_shape_residual_artifact"] = (
            build_focused_shape_residual_learner(
                focused_shape_basis,
                config,
    ...
    if bool(
        (cal_cfg.get("operational_residual_learner", {}) or {}).get("enabled", False)
    ):
    ...
    daily_peak_cfg = (
        (cal_cfg.get("stage_selector", {}) or {}).get("daily_peak_shadow_model", {})
        or {}
    ...
    hot_ramp_cfg = cal_cfg.get("hot_ramp_peak_capture", {}) or {}
    if bool(hot_ramp_cfg.get("enabled", False)):
        hot_ramp_basis = _apply_v126_correction_chain_to_frame(
            raw_df=raw_backtest_df,
            config=config,
            targeted_meta_artifact=artifacts["targeted_meta_artifact"],
            lookup_bundle=artifacts["lookup_bundle"],
        ...
        if artifacts.get("operational_residual_artifact") is not None:
            hot_ramp_basis = apply_operational_residual_learner(
                hot_ramp_basis,
                artifacts.get("operational_residual_artifact"),
                config,
                forecast_col="Final_Backtest_Forecast_MWH",
    ...
    heat_persistence_cfg = cal_cfg.get("heat_persistence_peak_capture", {}) or {}
    if bool(heat_persistence_cfg.get("enabled", False)):
        heat_persistence_basis = _apply_v126_correction_chain_to_frame(
            raw_df=raw_backtest_df,
            config=config,
            targeted_meta_artifact=artifacts["targeted_meta_artifact"],
            lookup_bundle=artifacts["lookup_bundle"],
        ...
        if artifacts.get("operational_residual_artifact") is not None:
            heat_persistence_basis = apply_operational_residual_learner(
...
def apply_origin_available_correction_chain(
    raw_df: pd.DataFrame, config: dict, artifacts: dict | None
) -> pd.DataFrame:
    ...
    out = apply_operational_stage_selector(
        out, config=config, forecast_col="Final_Backtest_Forecast_MWH"
    )
    final_col = "Final_Backtest_Forecast_MWH"
    out["Final_Residual_MWH"] = pd.to_numeric(
        out["Actual_MWH"], errors="coerce"
    ) - pd.to_numeric(out[final_col], errors="coerce")
...
def _configured_numeric_lookup(
    ...
    return numeric.map(
        lambda x: lookup.get(str(int(x)), default) if pd.notna(x) else default
    ).astype(float)
...
def apply_operational_stage_selector(
    df: pd.DataFrame, config: dict, forecast_col: str
) -> pd.DataFrame:
    ...
    if not bool(selector_cfg.get("enabled", True)):
        out["Stage_Selected_Forecast_MWH"] = pd.to_numeric(
            out[base_col], errors="coerce"
        )
    ...
    horizon_policy_enabled = bool(
        explicit_forecast_day or "Actual_MWH" not in out.columns
    )
    hour = pd.to_numeric(
    ...
    hot_peak = hour.between(16, 20) & daily_max.ge(
        float(selector_cfg.get("hot_peak_min_maxtemp_f", 90.0))
    )
    ...
    suppress_bias_for_overrides = bool(
        selector_cfg.get("suppress_bias_uplift_for_stage_overrides", False)
    )
    suppress_hot_for_overrides = bool(
        selector_cfg.get("suppress_hot_uplift_for_stage_overrides", False)
    )
    ...
    if not horizon_policy_enabled:
        allow_bias_uplift.loc[:] = bool(
            selector_cfg.get("non_horizon_apply_bias_uplift", False)
        )
        allow_hot_uplift.loc[:] = bool(
            selector_cfg.get("non_horizon_apply_hot_uplift", False)
        )
    ...
    def _stage_values(stage_name: str, fallback_col: str) -> tuple[str, pd.Series]:
        ...
        if (
            stage in {"targeted", "targeted_meta", "targeted_meta_adjusted"}
            and targeted_col in out.columns
        ):
            return targeted_col, pd.to_numeric(out[targeted_col], errors="coerce")
        if (
            stage in {"peak", "peak_risk", "peak_risk_adjusted"}
            and peak_col in out.columns
        ):
        ...
        if (
            stage in {"warm", "warm_ramp", "warm_ramp_adjusted"}
            and warm_col in out.columns
        ):
            return warm_col, pd.to_numeric(out[warm_col], errors="coerce")
        if (
            stage in {"cloud", "cloud_solar", "cloud_solar_adjusted"}
            and cloud_col in out.columns
        ):
    ...
    def _conditional_allowed_mask(
        values: pd.Series, allowed, *, numeric: bool = True
    ) -> pd.Series:
        if allowed is None:
    ...
    def _season_values(month: pd.Series) -> pd.Series:
        if "Season" in out.columns:
            season = (
                out["Season"].where(pd.notna(out["Season"]), "").astype(str).str.strip()
            )
    ...
    def _conditional_stage_mask(rule: dict) -> pd.Series:
        ...
        temp_bucket = pd.to_numeric(
            out.get(
                "DailyMaxTempBucket",
                out.get("DailyMaxTempBin", pd.Series(np.nan, index=out.index)),
            ),
            errors="coerce",
        )
        ...
        min_temp_bucket = rule.get(
            "min_daily_max_temp_bucket", rule.get("min_daily_max_temp_bin")
        )
        if min_temp_bucket is not None:
            mask &= temp_bucket.ge(float(min_temp_bucket))
        max_temp_bucket = rule.get(
            "max_daily_max_temp_bucket", rule.get("max_daily_max_temp_bin")
        )
    ...
    if day1.any():
        day1_stage = (
            str(selector_cfg.get("day1_stage", "blend_raw_peak")).strip().lower()
        )
        if day1_stage in {"raw", "raw_forecast"}:
            ...
            selected.loc[day1] = (
                raw.loc[day1] * (1.0 - peak_weight) + peak.loc[day1] * peak_weight
            )
    ...
    if d23.any():
        selected_col, values = _stage_values(
            selector_cfg.get("days2to3_stage", "peak_risk"), peak_col
        )
        selected.loc[d23] = values.loc[d23]
        source.loc[d23] = selected_col
        reason.loc[d23] = "days2to3_" + str(
            selector_cfg.get("days2to3_stage", "peak_risk")
        )
    ...
    if d47.any():
        selected_col, values = _stage_values(
            selector_cfg.get("days4to7_stage", "peak_risk"), peak_col
        )
        selected.loc[d47] = values.loc[d47]
        source.loc[d47] = selected_col
        reason.loc[d47] = "days4to7_" + str(
            selector_cfg.get("days4to7_stage", "peak_risk")
        )
        _mark_stage_override(d47)

    d8p = (
        horizon_policy_enabled
        & day.ge(8)
        & out.get(residual_col, pd.Series(np.nan, index=out.index)).notna()
    )
    if d8p.any():
    ...
    if (
        horizon_policy_enabled
        and bool(selector_cfg.get("hot_peak_final_stack_override", True))
        and hot_peak.any()
        and base_col in out.columns
    ):
        selected.loc[hot_peak] = pd.to_numeric(
            out.loc[hot_peak, base_col], errors="coerce"
        )
    ...
    if horizon_policy_enabled and bool(peak_window_cfg.get("enabled", False)):
        peak_hours = [
            int(h) for h in peak_window_cfg.get("hours", [14, 15, 16, 17, 18])
        ]
        ...
        if peak_window.any():
            selected_col, values = _stage_values(
                peak_window_cfg.get("stage", "warm_ramp"), warm_col
            )
            selected.loc[peak_window] = values.loc[peak_window]
            source.loc[peak_window] = selected_col
            reason.loc[peak_window] = (
                reason.loc[peak_window].astype(str) + "+peak_window_stage_override"
            )
    ...
    if horizon_policy_enabled and cloud_stage_cfgs:
        cloud = pd.to_numeric(
            out.get("CloudCover_Norm", pd.Series(np.nan, index=out.index)),
            errors="coerce",
        )
        solar_loss = pd.to_numeric(
            out.get(
                "BTM_Solar_Loss_From_ClearSky_MW", pd.Series(np.nan, index=out.index)
            ),
            errors="coerce",
        )
        if solar_loss.isna().all() and "Midday_Overcast_Solar_Loss_MW" in out.columns:
            solar_loss = pd.to_numeric(
                out["Midday_Overcast_Solar_Loss_MW"], errors="coerce"
            )
        for override_name, cloud_override_cfg in cloud_stage_cfgs:
            ...
            override_hours = [
                int(h)
                for h in cloud_override_cfg.get("hours", [10, 11, 12, 13, 14, 15, 16])
            ]
            ...
            cloud_override = hour.isin(override_hours) & (
                cloud.ge(min_cloud) | solar_loss.ge(min_loss)
            )
            ...
            if "min_maxtemp_f" in cloud_override_cfg:
                cloud_override &= daily_max.ge(
                    float(cloud_override_cfg["min_maxtemp_f"])
                )
            if "max_maxtemp_f" in cloud_override_cfg:
                cloud_override &= daily_max.lt(
                    float(cloud_override_cfg["max_maxtemp_f"])
                )
            if cloud_override.any():
                selected_col, values = _stage_values(
                    cloud_override_cfg.get("stage", "raw"), raw_col
                )
                selected.loc[cloud_override] = values.loc[cloud_override]
                source.loc[cloud_override] = selected_col
                reason.loc[cloud_override] = (
                    reason.loc[cloud_override].astype(str) + f"+{override_name}"
                )
    ...
    long_horizon_cfg = (
        selector_cfg.get("long_horizon_peak_hot_month_correction", {}) or {}
    )
    if horizon_policy_enabled and bool(long_horizon_cfg.get("enabled", False)):
        ...
        peak_hours = [
            int(h) for h in long_horizon_cfg.get("peak_hours", [14, 15, 16, 17, 18])
        ]
        hot_hours = [
            int(h) for h in long_horizon_cfg.get("hot_hours", [16, 17, 18, 19, 20])
        ]
        ...
        if "IsHoliday" in out.columns:
            is_holiday = (
                pd.to_numeric(out["IsHoliday"], errors="coerce").fillna(0).ne(0)
            )
        else:
        ...
        if peak_month_mask.any():
            selected.loc[peak_month_mask] = (
                selected.loc[peak_month_mask] + peak_corr.loc[peak_month_mask]
            )
            out.loc[peak_month_mask, "Long_Horizon_Peak_Month_Correction_MWH"] = (
                peak_corr.loc[peak_month_mask]
            )
            reason.loc[peak_month_mask] = (
                reason.loc[peak_month_mask].astype(str)
                + "+long_horizon_peak_month_correction"
            )
        if hot_month_mask.any():
            ...
            selected.loc[hot_month_mask] = (
                selected.loc[hot_month_mask] + hot_corr.loc[hot_month_mask]
            )
            out.loc[hot_month_mask, "Long_Horizon_Hot_Month_Correction_MWH"] = (
                hot_corr.loc[hot_month_mask]
            )
            reason.loc[hot_month_mask] = (
                reason.loc[hot_month_mask].astype(str)
                + "+long_horizon_hot_month_correction"
            )

    underforecast_uplift = float(
        selector_cfg.get("underforecast_bias_uplift_mwh", 0.0) or 0.0
    )
    if abs(underforecast_uplift) > 1e-9:
        selected.loc[allow_bias_uplift] = (
            selected.loc[allow_bias_uplift] + underforecast_uplift
        )
        reason.loc[allow_bias_uplift] = (
            reason.loc[allow_bias_uplift].astype(str) + "+underforecast_bias_uplift"
        )

    hot_uplift_cfg = selector_cfg.get("hot_peak_uplift", {}) or {}
    if bool(hot_uplift_cfg.get("enabled", False)):
        uplift_hours = [
            int(h) for h in hot_uplift_cfg.get("hours", [14, 15, 16, 17, 18, 19, 20])
        ]
        ...
        uplift = np.maximum(horizon_uplift, temp_uplift).clip(
            lower=0.0, upper=max_uplift
        )
        uplift_mask = (
            day.between(min_day, max_day)
        ...
        if uplift_mask.any():
            selected.loc[uplift_mask] = (
                selected.loc[uplift_mask] + uplift.loc[uplift_mask]
            )
            reason.loc[uplift_mask] = (
                reason.loc[uplift_mask].astype(str) + "+guarded_hot_peak_uplift"
            )
    ...
    if horizon_policy_enabled and bool(ultra_heat_cfg.get("enabled", False)):
        ...
        if ramp_full > min_maxtemp:
            ramp_scale = ((daily_max - min_maxtemp) / (ramp_full - min_maxtemp)).clip(
                lower=0.0, upper=1.0
            )
        else:
        ...
        if uplift_mask.any():
            selected.loc[uplift_mask] = (
                selected.loc[uplift_mask] + hour_uplift.loc[uplift_mask]
            )
            reason.loc[uplift_mask] = (
                reason.loc[uplift_mask].astype(str) + "+ultra_extreme_heat_wave_uplift"
            )
    ...
    if horizon_policy_enabled and conditional_overrides:
        for raw_rule in conditional_overrides:
            ...
            if bool(rule.get("only_if_stage_above_current", False)):
                min_advantage = float(rule.get("min_stage_advantage_mwh", 0.0) or 0.0)
                override_mask &= values.gt(selected + min_advantage).fillna(False)
                if not override_mask.any():
                    continue
            selected.loc[override_mask] = values.loc[override_mask]
            source.loc[override_mask] = selected_col
            name = (
                str(rule.get("name", "conditional_stage_override")).strip()
                or "conditional_stage_override"
            )
            reason.loc[override_mask] = (
                reason.loc[override_mask].astype(str)
                + f"+conditional_stage_override:{name}"
            )
...
def _apply_future_correction_chain(
    ...
    if bool(targeted_meta_cfg.get("enabled", True)):
        ...
        cal_future["Targeted_Meta_Adjusted_Forecast_MWH"] = (
            cal_future["Raw_Forecast_MWH"].astype(float).clip(lower=0.0)
        )
    ...
    if bool(cal_cfg.get("seasonal_enabled", True)):
        ...
        cal_future["Residual_Calibrated_Forecast_MWH"] = (
            cal_future[calibration_base_col].astype(float).clip(lower=0.0)
        )
        cal_future["Calibrated_Forecast_MWH"] = cal_future[
    ...
    if bool(cal_cfg.get("cloud_solar_shape_enabled", True)):
        cal_future = apply_cloud_solar_shape_correction(
            ...
            use_event_multiplier=bool(
                cal_cfg.get("cloud_solar_event_multiplier_enabled", True)
            ),
        )
    if bool(((cal_cfg.get("peak_risk", {}) or {}).get("enabled", True))):
        cal_future = apply_peak_risk_correction(
            cal_future, config=config, base_col="Calibrated_Forecast_MWH"
        )
    if bool((cal_cfg.get("recent_residual", {}) or {}).get("enabled", True)):
        ...
        cal_future["Recent_Corrected_Forecast_MWH"] = cal_future[
            "Calibrated_Forecast_MWH"
        ]
        cal_future["Final_Forecast_MWH"] = cal_future["Calibrated_Forecast_MWH"]
    cal_future = apply_operational_stage_selector(
        cal_future, config=config, forecast_col="Final_Forecast_MWH"
    )
    return cal_future
...
def run_pipeline(
    ...
    _progress(
        progress_callback, "Starting forecast pipeline", total=21 + len(scenario_defs)
    )
    ...
    if not local_wx.empty:
        local_temp_lookup, local_temp_matched = build_temperature_bias_lookup(
            hist_wx, local_wx, config
        )
        hist_wx = merge_local_station_temperature(hist_wx, local_wx)
        temp_cal_cfg = (config.get("local_weather", {}) or {}).get(
            "temperature_calibration", {}
        ) or {}
        if bool(temp_cal_cfg.get("enabled", False)) and not local_temp_lookup.empty:
            hist_wx = apply_temperature_bias_calibration(
                hist_wx, local_temp_lookup, config
            )
            fut_wx = apply_temperature_bias_calibration(
                fut_wx, local_temp_lookup, config
            )
    ...
    official_hourly_latest_dt = (
        load_df["DT"].max() if "DT" in load_df.columns and not load_df.empty else pd.NaT
    )
    ...
    five_min_hourly = build_hourly_load_from_five_min(
        five_min_load,
        timezone=str(
            config.get("project", {}).get("timezone") or "America/Los_Angeles"
        ),
        min_intervals_per_hour=int(
            five_min_cfg.get("min_completed_intervals_per_hour", 10)
        ),
    )
    respect_five_min_cutoff = bool(
        five_min_cfg.get("respect_actuals_import_cutoff", True)
    )
    if respect_five_min_cutoff:
        five_min_hourly = _filter_to_actuals_cutoff(five_min_hourly, actuals_cutoff_dt)
    if (
        bool(five_min_cfg.get("use_as_recent_hourly_load", True))
        and not five_min_hourly.empty
    ):
        ...
        hist_wx = _extend_historical_weather_with_recent_forecast(
            ...
            latest_load_dt=(
                load_df["DT"].max()
                if "DT" in load_df.columns and not load_df.empty
                else None
            ),
        )
    ...
    if respect_five_min_cutoff:
        intraday_features = _filter_to_actuals_cutoff(
            intraday_features, actuals_cutoff_dt, offset_hours=1.0
        )
    ...
    operational_residual_artifact = correction_artifacts.get(
        "operational_residual_artifact"
    )
    focused_shape_residual_artifact = correction_artifacts.get(
        "focused_shape_residual_artifact"
    )
    daily_peak_shadow_artifact = correction_artifacts.get("daily_peak_shadow_artifact")
    hot_ramp_peak_capture_artifact = correction_artifacts.get(
        "hot_ramp_peak_capture_artifact"
    )
    heat_persistence_peak_capture_artifact = correction_artifacts.get(
        "heat_persistence_peak_capture_artifact"
    )
    ...
    backtest_df = _apply_v126_correction_chain_to_frame(
        ...
        hot_ramp_peak_capture_artifact=hot_ramp_peak_capture_artifact,
        heat_persistence_peak_capture_artifact=heat_persistence_peak_capture_artifact,
    )
    ...
    xgb_model, lgb_model, xgb_feats = train_tree_models(
        train_df, features, config=config, stage_name="final full-history"
    )
    _progress(progress_callback, "Trained final tree models", advance=1)

    _progress(progress_callback, "Training benchmark models")
    prophet_fit = (
        train_prophet(train_df, DEFAULT_PROPHET_REGRESSORS, config=config)
    ...
    catboost_model, catboost_features = (
        train_catboost(train_df, xgb_feats, config=config)
    ...
    future_frame = build_forecast_frame(
        ...
        btm_monthly,
        intraday_load_features=intraday_features,
        max_intraday_carry_forward_hours=int(
            five_min_cfg.get("max_carry_forward_hours", 24)
        ),
        use_future_intraday_load_features=bool(
            five_min_cfg.get("future_model_features_enabled", False)
        ),
        solar_df=solar_df,
    )
    ...
    for scenario in scenario_defs:
        ...
        cal_future[col] = pd.to_numeric(
            scenario_cal["Final_Forecast_MWH"], errors="coerce"
        ).to_numpy()
    ...
    cal_future = apply_weather_scenario_delta_caps(
        cal_future, scenario_columns, config=config
    )
    ...
    cal_future = apply_hot_ramp_scenario_override(
        cal_future,
        config=config,
        base_col="Final_Forecast_MWH",
    ...
    cal_future = apply_day1_live_ramp_override(
        cal_future,
        train_df,
        config=config,
        base_col="Final_Forecast_MWH",
    ...
    cal_future = apply_multiday_live_heat_anchor_override(
        cal_future,
        config=config,
        base_col="Final_Forecast_MWH",
        also_update_cols=("Stage_Selected_Forecast_MWH", "Calibrated_Forecast_MWH"),
    )
    cal_future = apply_hot_ramp_peak_capture(
        cal_future,
        hot_ramp_peak_capture_artifact,
        config,
        forecast_col="Final_Forecast_MWH",
    ...
    cal_future = apply_heat_persistence_peak_capture(
        cal_future,
        heat_persistence_peak_capture_artifact,
        config,
        forecast_col="Final_Forecast_MWH",
    ...
    cal_future = apply_extreme_heat_magnitude_correction(
        cal_future,
        config,
        forecast_col="Final_Forecast_MWH",
        also_update_cols=("Stage_Selected_Forecast_MWH", "Calibrated_Forecast_MWH"),
    ...
    residual_band_lookup = (
        ...
        if bool(bands_cfg.get("residual_based", True))
        else None
    )
    ...
    diagnostics["operational_residual_learner_summary"] = (
        operational_residual_learner_summary(
            backtest_df,
            operational_residual_artifact,
            config,
    ...
    diagnostics["hot_ramp_peak_capture_summary"] = hot_ramp_peak_capture_summary(
        backtest_df,
        hot_ramp_peak_capture_artifact,
        config,
    )
    diagnostics["heat_persistence_peak_capture_summary"] = (
        heat_persistence_peak_capture_summary(
            backtest_df,
            heat_persistence_peak_capture_artifact,
            config,
    ...
    diagnostics["local_weather_temperature_bias_summary"] = (
        local_temperature_bias_summary(local_temp_matched, local_temp_lookup)
    )
    diagnostics["local_weather_temperature_bias_lookup"] = local_temp_lookup
    diagnostics["forecast_weather_used"] = (
        fut_wx.copy() if isinstance(fut_wx, pd.DataFrame) else pd.DataFrame()
    )
    diagnostics["forecast_weather_snapshot_summary"] = {
        "source": str(getattr(fut_wx, "attrs", {}).get("weather_source", "")),
        "snapshot_path": str(
            getattr(fut_wx, "attrs", {}).get("weather_snapshot_path", "")
        ),
        "cache_path": str(getattr(fut_wx, "attrs", {}).get("weather_cache_path", "")),
        "daily_cache_path": str(
            getattr(fut_wx, "attrs", {}).get("weather_daily_cache_path", "")
        ),
        "import_window_local": str(
            getattr(fut_wx, "attrs", {}).get("weather_import_window_local", "")
        ),
        "rows": int(len(fut_wx)) if isinstance(fut_wx, pd.DataFrame) else 0,
        "first_dt": (
            str(fut_wx["DT"].min())
            if isinstance(fut_wx, pd.DataFrame)
            and "DT" in fut_wx.columns
            and not fut_wx.empty
            else None
        ),
        "last_dt": (
            str(fut_wx["DT"].max())
    ...
    diagnostics["weather_stress_test_summary"] = build_weather_stress_summary(
        final_future, scenario_columns
    )
    diagnostics["five_min_hourly_load_debug"] = five_min_hourly
    if not five_min_hourly.empty:
        added = five_min_hourly[
            pd.to_datetime(five_min_hourly["DT"], errors="coerce")
            > official_hourly_latest_dt
        ].copy()
        diagnostics["five_min_hourly_load_summary"] = {
            ...
            "actuals_import_cutoff_dt": (
                str(actuals_cutoff_dt) if actuals_cutoff_dt is not None else ""
            ),
    ...
    return {
        ...
        "backtest_metrics": (
            backtest_df.attrs.get("metrics", {}) if backtest_df is not None else {}
        ),
...
def _apply_pre_cloud_correction_chain_to_frame(
    ...
    if bool((cal_cfg.get("targeted_residual_meta", {}) or {}).get("enabled", True)):
        out = apply_targeted_residual_meta_correction(
            raw_df, targeted_meta_artifact, config
        )
    else:
        out = raw_df.copy()
        out["Targeted_Meta_Adjusted_Forecast_MWH"] = (
            out["Raw_Forecast_MWH"].astype(float).clip(lower=0.0)
        )
    ...
    if bool(cal_cfg.get("seasonal_enabled", True)):
        ...
        out["Residual_Calibrated_Forecast_MWH"] = (
            out[base_col].astype(float).clip(lower=0.0)
        )
...
def _apply_v126_correction_chain_to_frame(
    ...
    hot_ramp_peak_capture_artifact: dict | None = None,
    heat_persistence_peak_capture_artifact: dict | None = None,
) -> pd.DataFrame:
    ...
    if bool(((cal_cfg.get("peak_risk", {}) or {}).get("enabled", True))):
        out = apply_peak_risk_correction(
            out, config=config, base_col="Calibrated_Forecast_MWH"
        )

    if simulate_recent and bool(
        (cal_cfg.get("recent_residual", {}) or {}).get("enabled", True)
    ):
        out = simulate_recent_residual_correction_backtest(
            out, config=config, base_col="Calibrated_Forecast_MWH"
        )
    else:
    ...
    if (
        bool(daily_peak_cfg.get("enabled", False))
        or daily_peak_shadow_artifact is not None
    ):
    ...
    hot_ramp_cfg = cal_cfg.get("hot_ramp_peak_capture", {}) or {}
    if (
        bool(hot_ramp_cfg.get("enabled", False))
        or hot_ramp_peak_capture_artifact is not None
    ):
    ...
    heat_persistence_cfg = cal_cfg.get("heat_persistence_peak_capture", {}) or {}
    if (
        bool(heat_persistence_cfg.get("enabled", False))
        or heat_persistence_peak_capture_artifact is not None
    ):
...
def build_display_df(train_df: pd.DataFrame, future_df: pd.DataFrame) -> pd.DataFrame:
    ...
    forecast_col = (
        "Final_Forecast_MWH"
        if "Final_Forecast_MWH" in future_df.columns
        else "Calibrated_Forecast_MWH"
    )
    fut_cols = [
        "DT",
        ...
        "Prophet_Lower_MWH",
        "Prophet_Upper_MWH",
        "Targeted_Meta_Bias_Cal_MWH",
        "Targeted_Meta_SolarCloud_Cal_MWH",
        "Targeted_Meta_Cal_MWH",
        "Residual_Cal_MWH",
        "Heat_Peak_Cal_MWH",
        "Warm_Ramp_Cal_MWH",
        ...
        "Hot_Ramp_Override_Gate",
        "Hot_Ramp_Override_HeatPersistence_Flag",
        "Hot_Ramp_Override_PeakRisk_Protected_MWH",
        "Hot_Ramp_Override_Scenario_Gap_MWH",
        "Hot_Ramp_Override_Scenario_Lift_MWH",
        "Hot_Ramp_Override_Cal_MWH",
        "Hot_Ramp_Override_Source",
        "Day1_Live_Ramp_Gate",
        "Day1_Live_Ramp_Cal_MWH",
        "Day1_Live_Ramp_Observed_Delta_MWH",
        "Day1_Live_Ramp_Target_Delta_MWH",
        "Day1_Live_Ramp_Target_MWH",
        "Day1_Live_Ramp_Yesterday_MWH",
        "Day1_Live_Ramp_Latest_Actual_Hour",
        "Day1_Live_Ramp_Pair_Count",
        "Day1_Live_Ramp_Source",
        "MultiDay_Heat_Anchor_Gate",
        "MultiDay_Heat_Anchor_Cal_MWH",
        "MultiDay_Heat_Anchor_Target_MWH",
        "MultiDay_Heat_Anchor_Anchor_Peak_MWH",
        "MultiDay_Heat_Anchor_Anchor_Temp_F",
        "MultiDay_Heat_Anchor_Observed_Delta_MWH",
        "MultiDay_Heat_Anchor_Lead_Day",
        "MultiDay_Heat_Anchor_Weekend_Discount_MWH",
        "MultiDay_Heat_Anchor_Source",
        "AR_Residual_Correction_MWH",
        "AR_Residual_Phi",
        "AR_Residual_Latest_MWH",
        "OriginDay_State_Correction_MWH",
        "OriginDay_State_MWH",
        "OriginDay_Latest_Day_MWH",
        "Band",
        "Upper_Band",
        "Lower_Band",
        "P10_Forecast_MWH",
        ...
        "Weather_Input_Risk_Multiplier",
        "Weather_Input_Risk_Reason",
        "Weather_Input_Risk_Class",
        "Production_Caution_Flag",
        "Production_Caution_Reason",
        "Production_Confidence_Label",
        "Production_Risk_Code",
        "Pre_Conformal_Band_MWH",
        "Conformal_Weather_Band_MWH",
        "Conformal_Weather_Source",
        "WeatherScenario_Min_P50_MWH",
        "WeatherScenario_Max_P50_MWH",
        ...
        *HEAT_PERSISTENCE_PEAK_COLUMNS,
        "Calibration_Level",
        "Calibration_Matched_Levels",
        "Targeted_Meta_Source",
        "Warm_Ramp_Correction_Source",
        "Cloud_Solar_Correction_Source",
        "Peak_Risk_Source",
        "Recent_Correction_Source",
        "AR_Residual_Source",
        "OriginDay_State_Source",
    ...
    scenario_cols = [
        ...
        if c.startswith("WeatherScenario_") and c.endswith("_P50_MWH")
    ]
    selected_cols = list(
        dict.fromkeys(c for c in fut_cols + scenario_cols if c in future_df.columns)
    )
