...
from forecasting.features.weather_features import (
    add_delta_breeze_weather_shape_features,
    add_heat_persistence_features,
)
...
def _recompute_weather_temperature(out: pd.DataFrame) -> pd.DataFrame:
    ...
    if "IsLikelySystemPeakHour" in out.columns:
        out["Cooling_Stress"] = out["CDD"] * pd.to_numeric(
            out["IsLikelySystemPeakHour"], errors="coerce"
        ).fillna(0.0)
        out["DailyMax_x_PeakHour"] = out["Temperature_DailyMax"] * pd.to_numeric(
            out["IsLikelySystemPeakHour"], errors="coerce"
        ).fillna(0.0)
    out["Humidity_x_Temp"] = humidity * temp
    if "WindSpeed_Mph" in out.columns:
        out["Wind_x_Temp"] = (
            pd.to_numeric(out["WindSpeed_Mph"], errors="coerce").fillna(0.0) * temp
        )
...
def _recompute_solar_cloud(out: pd.DataFrame) -> pd.DataFrame:
    ...
    ghi = (
        pd.to_numeric(out.get("GHI_Wm2", ghi_default), errors="coerce")
        .fillna(0.0)
        .clip(lower=0.0)
    )
    ...
    if "BTM_ClearSky_Proxy_MW" in out.columns:
        out["BTM_Solar_Cloud_Adjusted_MW"] = pd.to_numeric(
            out.get("BTM_Solar_Proxy_MW"), errors="coerce"
        ).fillna(0.0)
    ...
    if "ClearSky_GHI_Proxy_Wm2" in out.columns:
        denom = pd.to_numeric(out["ClearSky_GHI_Proxy_Wm2"], errors="coerce").replace(
            0.0, np.nan
        )
        out["ClearSky_Index"] = (
            (ghi / denom).replace([np.inf, -np.inf], np.nan).clip(0.0, 1.35).fillna(0.0)
        )
        out["Cloud_x_ClearSky_GHI"] = cloud * pd.to_numeric(
            out["ClearSky_GHI_Proxy_Wm2"], errors="coerce"
        ).fillna(0.0)
    if "Solar_Hour_Shape" in out.columns:
        out["Cloud_x_Solar_Hour"] = cloud * pd.to_numeric(
            out["Solar_Hour_Shape"], errors="coerce"
        ).fillna(0.0)
    out["Cloud_x_GHI"] = cloud * ghi
    if "BTM_Solar_Proxy_MW" in out.columns:
        out["BTM_x_GHI"] = (
            pd.to_numeric(out["BTM_Solar_Proxy_MW"], errors="coerce").fillna(0.0) * ghi
        )
        out["BTM_x_Cloud"] = (
            pd.to_numeric(out["BTM_Solar_Proxy_MW"], errors="coerce").fillna(0.0)
            * cloud
        )
        if "Solar_Midday_Flag" in out.columns:
            out["BTM_Midday_Impact"] = out["BTM_Solar_Proxy_MW"] * pd.to_numeric(
                out["Solar_Midday_Flag"], errors="coerce"
            ).fillna(0.0)
        if "Solar_Evening_Ramp_Flag" in out.columns:
            out["BTM_Evening_Ramp_Impact"] = out["BTM_Solar_Proxy_MW"] * pd.to_numeric(
                out["Solar_Evening_Ramp_Flag"], errors="coerce"
            ).fillna(0.0)
    if "Date" not in out.columns:
        out["Date"] = pd.to_datetime(out["DT"]).dt.date
    if "BTM_Solar_Proxy_MW" in out.columns:
        out["Daily_BTM_Solar_Proxy_Total_MWh"] = out.groupby("Date")[
            "BTM_Solar_Proxy_MW"
        ].transform("sum")
        out["Daily_BTM_Solar_Proxy_Max_MW"] = out.groupby("Date")[
            "BTM_Solar_Proxy_MW"
        ].transform("max")
    ...
    if "BTM_Solar_Loss_From_ClearSky_MW" in out.columns:
        out["Daily_BTM_Solar_Loss_MWh"] = out.groupby("Date")[
            "BTM_Solar_Loss_From_ClearSky_MW"
        ].transform("sum")
        out["Daily_BTM_Solar_Loss_Max_MW"] = out.groupby("Date")[
            "BTM_Solar_Loss_From_ClearSky_MW"
        ].transform("max")
        midday = pd.to_numeric(
            out.get("Solar_Midday_Flag", pd.Series(0, index=out.index)), errors="coerce"
        ).fillna(0.0)
        out["Midday_Overcast_Solar_Loss_MW"] = (
            out["BTM_Solar_Loss_From_ClearSky_MW"] * midday * cloud.ge(0.60).astype(int)
        )
    return out


def make_weather_scenario_frame(
    base_future_frame: pd.DataFrame, scenario: dict[str, Any]
) -> pd.DataFrame:
    ...
    if cloud_delta and "CloudCover_Norm" in out.columns:
        out["CloudCover_Norm"] = (
            pd.to_numeric(out["CloudCover_Norm"], errors="coerce").fillna(0.0)
            + cloud_delta
        ).clip(0.0, 1.0)
...
def scenario_column_name(name: str) -> str:
    safe = "".join(
        ch if ch.isalnum() else "_" for ch in str(name).strip().lower()
    ).strip("_")
    return f"WeatherScenario_{safe}_P50_MWH"


def add_scenario_summary_columns(
    future_df: pd.DataFrame, scenario_columns: list[str]
) -> pd.DataFrame:
    out = future_df.copy()
    ...
    base = pd.to_numeric(
        out.get(base_col, pd.Series(np.nan, index=out.index)), errors="coerce"
    )
    ...
    out["WeatherScenario_Spread_MWH"] = (
        out["WeatherScenario_Max_P50_MWH"] - out["WeatherScenario_Min_P50_MWH"]
    )
...
def _forecast_day_index(out: pd.DataFrame) -> pd.Series:
    ...
    dt = pd.to_datetime(
        out.get("DT", pd.Series(pd.NaT, index=out.index)), errors="coerce"
    )
    if dt.dropna().empty:
...
def _scenario_delta_cap_series(df: pd.DataFrame, config: dict | None) -> pd.Series:
    ...
    default_cap = float(
        cap_cfg.get("default_mwh", cfg.get("scenario_delta_cap_mwh", 18.0))
    )
    ...
    cloud = pd.to_numeric(
        df.get("CloudCover_Norm", pd.Series(np.nan, index=df.index)), errors="coerce"
    )
    loss = pd.to_numeric(
        df.get("BTM_Solar_Loss_From_ClearSky_MW", pd.Series(np.nan, index=df.index)),
        errors="coerce",
    )
    day = _forecast_day_index(df)

    hot_peak = hour.between(16, 20) & temp_max.ge(
        float(cap_cfg.get("hot_peak_min_maxtemp_f", 90.0))
    )
    cloud_solar = hour.between(10, 16) & (
        cloud.ge(0.60) | loss.ge(float(cap_cfg.get("cloud_solar_min_loss_mw", 1.25)))
    )
...
def apply_weather_scenario_delta_caps(
    ...
    for col in cols:
        ...
        any_capped |= (
            delta.notna()
            & capped_delta.notna()
            & (delta.sub(capped_delta).abs() > 1e-9)
        )
...
def build_weather_stress_summary(
    ...
    cloud = pd.to_numeric(
        out.get("CloudCover_Norm", pd.Series(np.nan, index=out.index)), errors="coerce"
    )
    loss = pd.to_numeric(
        out.get("BTM_Solar_Loss_From_ClearSky_MW", pd.Series(np.nan, index=out.index)),
        errors="coerce",
    )
    ...
    cap_applied = (
        pd.to_numeric(
            out.get("WeatherScenario_Cap_Applied", pd.Series(0, index=out.index)),
            errors="coerce",
        )
        .fillna(0)
        .gt(0)
...
def latest_validation_detail_path(output_dir: str | Path) -> Path | None:
    ...
    files = sorted(
        path.glob("weather_interval_coverage_validation_detail_*.csv"),
...
def build_conformal_lookup(
    detail_path: str | Path, quantile: float = 0.80
) -> dict[str, Any]:
    ...
    if "Weather_Input_Risk_Class" in detail.columns:
        lookup["risk_class"] = (
            detail.groupby("Weather_Input_Risk_Class", dropna=False)["AbsError_MWH"]
            .quantile(float(quantile))
            .astype(float)
            .to_dict()
        )
    if "Replay_Horizon_Bucket" in detail.columns:
        lookup["horizon"] = (
            detail.groupby("Replay_Horizon_Bucket", dropna=False)["AbsError_MWH"]
            .quantile(float(quantile))
            .astype(float)
            .to_dict()
        )
    if {"Weather_Input_Risk_Class", "Replay_Horizon_Bucket"}.issubset(detail.columns):
        rh = detail.groupby(
            ["Weather_Input_Risk_Class", "Replay_Horizon_Bucket"], dropna=False
        )["AbsError_MWH"].quantile(float(quantile))
        lookup["risk_horizon"] = {
            "|".join(str(part) for part in key): float(value)
            for key, value in rh.items()
        }
    return lookup


def apply_conformal_weather_bands(
    df: pd.DataFrame, config: dict | None, output_dir: str | Path
) -> pd.DataFrame:
    cfg = ((config or {}).get("bands", {}) or {}).get("conformal_weather", {}) or {}
    out = df.copy()
    out["Pre_Conformal_Band_MWH"] = pd.to_numeric(
        out.get("Band", np.nan), errors="coerce"
    )
    if not bool(cfg.get("enabled", True)):
    ...
    lookup = build_conformal_lookup(
        detail_path, quantile=float(cfg.get("quantile", 0.80))
    )
    if not lookup:
        out["Conformal_Weather_Band_MWH"] = np.nan
    ...
    horizon = out.get("Replay_Horizon_Bucket", pd.Series("", index=out.index)).astype(
        str
    )
    if "Forecast_Day" in out.columns:
        day = pd.to_numeric(out["Forecast_Day"], errors="coerce")
    ...
    conformal = pd.Series(
        float(lookup.get("global", np.nan)), index=out.index, dtype=float
    )
    ...
    caution_reason = out.get(
        "Production_Caution_Reason", pd.Series("none", index=out.index)
    ).astype(str)
    high_risk = caution_reason.str.contains(
        "hot_peak|peak_window|cloudy_solar|days8to16", case=False, regex=True, na=False
    ) | risk.str.contains(
        "hot_peak|cloudy_solar|shoulder_heat|high_temp|days8to16",
        case=False,
        regex=True,
        na=False,
    )
