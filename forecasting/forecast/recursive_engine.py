...
def _rolling(
    values: list[float], window: int, min_periods: int | None = None, std: bool = False
) -> float:
...
def _row_float(row: pd.Series, col: str, default: float = 0.0) -> float:
    try:
        value = pd.to_numeric(pd.Series([row.get(col, default)]), errors="coerce").iloc[
            0
        ]
...
def _load_decay_shape_values(row: pd.Series) -> dict[str, float]:
    ...
    if not np.isfinite(clear_peak_16_18):
        clear_peak_16_18 = (
            peak_window_16_18 if np.isfinite(cloud) and cloud <= 0.10 else 0.0
        )
    ...
    if not np.isfinite(overcast_peak_16_18):
        overcast_peak_16_18 = (
            peak_window_16_18 if np.isfinite(cloud) and cloud >= 0.60 else 0.0
        )
    ...
    if not np.isfinite(clear_hot_peak_16_18):
        clear_hot_peak_16_18 = (
            clear_peak_16_18 if np.isfinite(daily_max) and daily_max >= 90.0 else 0.0
        )
    ...
    if not np.isfinite(overcast_cool_peak_16_18):
        overcast_cool_peak_16_18 = (
            overcast_peak_16_18 if np.isfinite(daily_max) and daily_max < 75.0 else 0.0
        )
    ...
    delta_flag = max(
        ...
        float(
            np.clip(_row_float(row, "DeltaBreeze_Westerly_Flow_Flag", 0.0), 0.0, 1.0)
        ),
        float(
            np.clip(_row_float(row, "DeltaBreeze_EveningWindRamp_Flag", 0.0), 0.0, 1.0)
        ),
    )
    ...
    lag1_minus_yesterday = (
        lag1 - lag24 if np.isfinite(lag1) and np.isfinite(lag24) else np.nan
    )
    lag1_minus_7day = (
        lag1 - same_hour_7day
        if np.isfinite(lag1) and np.isfinite(same_hour_7day)
        else np.nan
    )
    lag24_minus_7day = (
        lag24 - same_hour_7day
        if np.isfinite(lag24) and np.isfinite(same_hour_7day)
        else np.nan
    )
    vs_yesterday = lag24 - lag1 if np.isfinite(lag24) and np.isfinite(lag1) else np.nan
    vs_7day = (
        same_hour_7day - lag1
        if np.isfinite(same_hour_7day) and np.isfinite(lag1)
        else np.nan
    )
    ...
    post_vs_yesterday = (
        post_peak * vs_yesterday if np.isfinite(vs_yesterday) else np.nan
    )
    post_vs_7day = post_peak * vs_7day if np.isfinite(vs_7day) else np.nan
    clear_hot_vs_7day = (
        clear_hot * post_vs_7day if np.isfinite(post_vs_7day) else np.nan
    )
    ...
    return {
        ...
        "PeakWindow_Lag1_Minus_SameHour7DayMean_MWH": (
            peak_window * lag1_minus_7day if np.isfinite(lag1_minus_7day) else np.nan
        ),
        "PeakWindow_Lag24_Minus_SameHour7DayMean_MWH": (
            peak_window * lag24_minus_7day if np.isfinite(lag24_minus_7day) else np.nan
        ),
        "PeakWindow16to18_Lag1_Minus_SameHour7DayMean_MWH": (
            peak_window_16_18 * lag1_minus_7day
            if np.isfinite(lag1_minus_7day)
            else np.nan
        ),
        "PeakWindow16to18_Lag24_Minus_SameHour7DayMean_MWH": (
            peak_window_16_18 * lag24_minus_7day
            if np.isfinite(lag24_minus_7day)
            else np.nan
        ),
        "HotPeak_Lag1_Minus_SameHourYesterday_MWH": (
            hot_peak * lag1_minus_yesterday
            if np.isfinite(lag1_minus_yesterday)
            else np.nan
        ),
        "HotPeak_Lag1_Minus_SameHour7DayMean_MWH": (
            hot_peak * lag1_minus_7day if np.isfinite(lag1_minus_7day) else np.nan
        ),
        "HotPeak_Lag24_Minus_SameHour7DayMean_MWH": (
            hot_peak * lag24_minus_7day if np.isfinite(lag24_minus_7day) else np.nan
        ),
        "ClearHotPeak_Lag1_Minus_SameHourYesterday_MWH": (
            clear_hot_peak * lag1_minus_yesterday
            if np.isfinite(lag1_minus_yesterday)
            else np.nan
        ),
        "ClearHotPeak_Lag1_Minus_SameHour7DayMean_MWH": (
            clear_hot_peak * lag1_minus_7day if np.isfinite(lag1_minus_7day) else np.nan
        ),
        "ClearHotPeak_Lag24_Minus_SameHour7DayMean_MWH": (
            clear_hot_peak * lag24_minus_7day
            if np.isfinite(lag24_minus_7day)
            else np.nan
        ),
        "ClearPeak16to18_Lag24_Minus_SameHour7DayMean_MWH": (
            clear_peak_16_18 * lag24_minus_7day
            if np.isfinite(lag24_minus_7day)
            else np.nan
        ),
        "OvercastPeak16to18_Lag24_Minus_SameHour7DayMean_MWH": (
            overcast_peak_16_18 * lag24_minus_7day
            if np.isfinite(lag24_minus_7day)
            else np.nan
        ),
        "ClearHotPeak16to18_Lag24_Minus_SameHour7DayMean_MWH": (
            clear_hot_peak_16_18 * lag24_minus_7day
            if np.isfinite(lag24_minus_7day)
            else np.nan
        ),
        "OvercastCoolPeak16to18_Lag1_Minus_SameHour7DayMean_MWH": (
            overcast_cool_peak_16_18 * lag1_minus_7day
            if np.isfinite(lag1_minus_7day)
            else np.nan
        ),
        "OvercastCoolPeak16to18_Lag24_Minus_SameHour7DayMean_MWH": (
            overcast_cool_peak_16_18 * lag24_minus_7day
            if np.isfinite(lag24_minus_7day)
            else np.nan
        ),
...
def recursive_forecast(
    ...
    base_series = (
        pd.to_numeric(hist["MWH"], errors="coerce").dropna().astype(float).tolist()
    )

    prophet_components = pd.DataFrame(index=fut.index)
    if prophet_fit is not None:
        try:
            prophet_components = predict_prophet(
                prophet_fit, fut, prophet_features
            ).reset_index(drop=True)
        except Exception as exc:
            # Prophet should not break the whole forecast if the tree ensemble is healthy.
            print(
                f"WARNING: Prophet prediction failed; continuing with XGB/LGB only. Details: {exc}"
            )
            prophet_components = pd.DataFrame(index=fut.index)
    ...
    for i in range(len(fut)):
        ...
        prop_p = np.nan
        if "Prophet_Pred_MWH" in prophet_components.columns and i < len(
            prophet_components
        ):
        ...
        if catboost_model is not None:
            try:
                ...
                if i == 0:
                    print(
                        f"WARNING: CatBoost prediction failed; continuing without CatBoost benchmark. Details: {exc}"
                    )
                cat_p = np.nan
        ...
        pred = (
            float(pred_arr[0])
            if len(pred_arr) and np.isfinite(pred_arr[0])
            else np.nanmean([xgb_p, lgb_p])
        )
        pred = max(0.0, pred)
