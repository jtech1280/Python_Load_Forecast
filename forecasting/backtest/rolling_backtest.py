...
def _safe_mape(actual: pd.Series, forecast: pd.Series) -> float:
    ...
    mask = a.abs() > 1e-9
    return (
        float((np.abs((a[mask] - f[mask]) / a[mask])).mean() * 100.0)
        if mask.any()
        else np.nan
    )
...
def run_rolling_backtest(
    ...
    bt_xgb, bt_lgb, bt_features = train_tree_models(
        hist, features, config=config, stage_name="backtest holdout"
    )
    _record_timing(
        timing_rows,
        stage="tree training",
        started=started,
        rows=len(hist),
        detail=f"features={len(bt_features)}",
    )
    ...
    if skip_prophet and prophet_enabled(config):
        ...
        _record_timing(
            ...
            started=started,
            status="skipped",
            detail="prophet_enabled=false",
        )
    bt_prophet_features = (
        bt_prophet_fit.regressors if bt_prophet_fit is not None else []
    )
    ...
    if skip_catboost:
        ...
        _record_timing(
            ...
            started=started,
            status="skipped",
            detail="skip_catboost=true",
        )
    else:
        bt_catboost, _ = (
            train_catboost(hist, bt_features, config=config)
            if catboost_enabled(config)
            else (None, bt_features)
        )
        _record_timing(
            timing_rows,
            stage="CatBoost training",
    ...
    fut_preds = recursive_forecast(
        ...
        bt_features,
        ensemble_weights,
        prophet_fit=bt_prophet_fit,
        prophet_features=bt_prophet_features,
        catboost_model=bt_catboost,
    )
    _record_timing(
        timing_rows, stage="recursive forecast", started=started, rows=len(fut_preds)
    )
    ...
    out["Residual_MWH"] = out["Actual_MWH"].astype(float) - out[
        "Raw_Forecast_MWH"
    ].astype(float)
    ...
    out.attrs["metrics"] = {
        ...
        "RMSE_MWH": (
            float(np.sqrt(np.mean(np.square(out["Residual_MWH"]))))
            if len(out)
            else np.nan
        ),
        ...
        "Peak_Error_MWH": (
            float(
                out.loc[out["Actual_MWH"].idxmax(), "Raw_Forecast_MWH"]
                - out["Actual_MWH"].max()
            )
            if len(out)
            else np.nan
