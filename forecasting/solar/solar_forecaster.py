...
REGIME_CLEAR_SKY_LABELS = ["csi_000_020", "csi_020_050", "csi_050_080", "csi_080_105", "csi_105_160"]
...
def _read_solar_weather_cache(
    ...
    *,
    ...
    if timestamp_col == "date":
        ...
        in_range = out[timestamp_col].notna() & out[timestamp_col].ge(start_date) & out[timestamp_col].le(end_date)
    else:
...
def classify_customer_segments(df: pd.DataFrame) -> pd.Series:
    ...
    """
    meter_type = df.get("MeterType", pd.Series("", index=df.index)).fillna("").astype(str).str.strip().str.upper()
    rate_schedule = df.get("RateSchedule", pd.Series("", index=df.index)).fillna("").astype(str).str.strip().str.upper()
    ...
    solar_20_meter = meter_type.apply(lambda value: any(pattern in value for pattern in SOLAR_20_METER_PATTERNS))
    solar_20_rate = rate_schedule.str.startswith("E2") | rate_schedule.str.contains("SL", na=False)
    segment.loc[(solar_20_meter | solar_20_rate) & ~nem_meter] = CUSTOMER_SEGMENT_SOLAR_20
    return segment


def parse_customer_segments(value: str) -> list[str]:
    segments = [segment.strip().upper() for segment in str(value).split(",") if segment.strip()]
    valid_segments = {CUSTOMER_SEGMENT_NEM, CUSTOMER_SEGMENT_SOLAR_20, CUSTOMER_SEGMENT_OTHER}
...
def aggregate_segment_export_to_total(segment_intervals: pd.DataFrame) -> pd.DataFrame:
    ...
    aggregations = {
        ...
        "ExportSource": ("ExportSource", lambda values: "+".join(sorted(set(values.dropna().astype(str))))),
    }
...
def weather_for_sites(weather_df: pd.DataFrame, sites: Optional[pd.DataFrame], use_capacity_weighted_weather: bool) -> pd.DataFrame:
    """
    Return one hourly weather series for the provided site subset.
    """
    if weather_df.empty:
        return weather_df.copy()
    if use_capacity_weighted_weather and sites is not None and "SolarSiteKey" in weather_df.columns:
...
def load_forecast_eligible_solar_sites(conn: Engine) -> pd.DataFrame:
    ...
    df["InterconnectionDate"] = pd.to_datetime(df["InterconnectionDate"], errors="coerce").dt.date
...
def calculate_solar_position(
    ...
    """
    timestamp_index = timestamps.index if hasattr(timestamps, "index") else pd.RangeIndex(len(timestamps))
    ...
    utc_offset_hours = local_timestamps.apply(
        lambda value: (
            value.to_pydatetime().replace(tzinfo=timezone).utcoffset().total_seconds() / 3600.0
            if pd.notna(value)
            else np.nan
        )
    )
...
def _parse_interval_parquet_metadata(path: Path, parquet_root: Path) -> dict[str, str] | None:
    try:
    ...
    month_part = next((part for part in parts if part.upper().startswith("MONTHLY_")), "")
    month = month_part.split("_", 1)[1] if "_" in month_part else ""
    if not (len(month) == 6 and month.isdigit()):
        return None

    channel = next((part.upper() for part in parts if part.upper() in {"DEL", "NET", "REC"}), "")
    if not channel:
        return None

    segment = upper_parts[0]
    rate_group_part = next((part for part in parts if part.upper().startswith("GS")), "RES")
    return {
...
def rebuild_rec_file_catalog(parquet_root: Path) -> pd.DataFrame:
    ...
    if not source_files:
        raise FileNotFoundError(f"No COM/RES interval parquet files found under {parquet_root}.")

    logging.info("Rebuilding interval parquet catalog from %s parquet files under %s", f"{len(source_files):,}", parquet_root)
    rows = []
    ...
    if not rows:
        raise FileNotFoundError(f"No recognizable COM/RES Monthly_YYYYMM interval parquet files found under {parquet_root}.")
    ...
    logging.info("Wrote interval parquet catalog: %s rows to %s", f"{len(catalog):,}", catalog_path)
    return catalog


def _normalize_rec_file_catalog(catalog: pd.DataFrame, parquet_root: Path) -> pd.DataFrame:
    """
        ...
        normalized_rows.append(
            {
                ...
                "nem_status": meta["nem_status"],
                "month": meta["month"],
                "size_mb": row.get("size_mb") if not _missing(row.get("size_mb")) else round(float(size_bytes) / (1024 * 1024), 3),
                "size_bytes": int(size_bytes),
                "modified_time": str(modified_time),
                "modified_time_ns": int(modified_time_ns),
                "columns_json": row.get("columns_json") if not _missing(row.get("columns_json")) else "[]",
                "schema_error": "" if _missing(row.get("schema_error", "")) else str(row.get("schema_error", "")),
            }
        )

    normalized = pd.DataFrame(normalized_rows, columns=CATALOG_COLUMNS)
    if skipped_rows:
        logging.info("Ignored %s non-interval parquet index rows outside COM/RES Monthly_YYYYMM folders", f"{skipped_rows:,}")
    return normalized
...
def _read_or_rebuild_rec_file_catalog(parquet_root: Path) -> pd.DataFrame:
    ...
    if catalog_path.exists():
        ...
        if normalized.empty:
            logging.warning("Interval parquet catalog at %s had no usable COM/RES interval rows; rebuilding it.", catalog_path)
        ...
        normalized = _normalize_rec_file_catalog(catalog, parquet_root)
        if normalized.empty:
            logging.warning("Interval parquet CSV index at %s had no usable COM/RES interval rows; rebuilding it.", csv_index_path)
            return rebuild_rec_file_catalog(parquet_root)
        _write_rec_file_catalog_cache(normalized, parquet_root)
        return normalized

    logging.warning("Interval parquet catalog cache is missing; rebuilding it under %s", parquet_root)
    return rebuild_rec_file_catalog(parquet_root)


def rebuild_spid_file_lookup(parquet_root: Path, catalog: pd.DataFrame | None = None) -> pd.DataFrame:
    """
    Rebuild ServicePointID-to-FileID lookup parts from the interval parquet files.
    """
    catalog = catalog.copy() if catalog is not None else _read_or_rebuild_rec_file_catalog(parquet_root)
    if catalog.empty:
        raise ValueError("Cannot rebuild service point lookup from an empty parquet catalog.")
    ...
    logging.info("Rebuilding service point file lookup from %s catalog files", f"{len(catalog):,}")
    ...
    for idx, row in enumerate(catalog.itertuples(index=False), start=1):
        ...
        try:
            ...
            read_errors += 1
            logging.warning("Skipping %s while rebuilding service point lookup: %s", path, exc)
            continue
        ...
        bucket = pd.util.hash_pandas_object(counts["SPID_BASE"], index=False).mod(256).astype(int)
        for bucket_id, group in counts.groupby(bucket):
            bucket_frames[int(bucket_id)].append(group.reset_index(drop=True))

        if idx % 500 == 0:
            logging.info("Indexed service points from %s / %s parquet files", f"{idx:,}", f"{len(catalog):,}")
    ...
    for bucket_id, frames in bucket_frames.items():
        ...
        part["RowCount"] = pd.to_numeric(part["RowCount"], errors="coerce").fillna(0).astype("int64")
    ...
    (_catalog_path(parquet_root).parent / "metadata.json").write_text(json.dumps(metadata, indent=2), encoding="utf-8")
    logging.info(
...
def load_spid_file_lookup(parquet_root: Path) -> pd.DataFrame:
    ...
    if not lookup_root.exists():
        logging.warning("Service point lookup cache is missing; rebuilding it under %s", parquet_root)
        return rebuild_spid_file_lookup(parquet_root)

    lookup_files = sorted(lookup_root.glob("bucket=*/*.parquet"))
    if not lookup_files:
        logging.warning("Service point lookup cache has no parquet parts; rebuilding it under %s", parquet_root)
        return rebuild_spid_file_lookup(parquet_root)
...
def load_rec_interval_data(
    ...
    if lookup.empty:
        raise ValueError("No forecast-eligible solar service points found in export parquet lookup for requested dates.")
    ...
    segment_by_spid = lookup.drop_duplicates("SPID").set_index("SPID")["CustomerSegment"].to_dict()
    ...
    for index, (file_id, service_points_in_file) in enumerate(spids_by_file.items(), start=1):
        ...
        if group_by_customer_segment:
            df["CustomerSegment"] = df["ServicePointID"].map(segment_by_spid).fillna(CUSTOMER_SEGMENT_OTHER)
    ...
    if group_by_customer_segment:
        segment_summary = (
            ...
            .agg(Export_MWh=("Export_kWh", lambda values: values.sum() / 1000.0), Peak_MW=("Export_kW", lambda values: values.max() / 1000.0))
...
def build_average_daily_shape(
    ...
    if method == "mean":
        ...
        if valid_daily_peaks.empty:
            ...
            if method == "median":
                normalized_shape = (
                    shape_df.groupby("time", as_index=False)["NormalizedPower"]
                    .median()
                )
...
def calculate_active_capacity_for_timestamps(
    ...
    fallback = pd.Series(float(default_capacity_kw), index=timestamp_series.index, dtype="float64")
    if (
    ...
    if "InterconnectionDate" in sites.columns:
        interconnection_series = pd.to_datetime(sites["InterconnectionDate"], errors="coerce")
    else:
        interconnection_series = pd.Series(pd.NaT, index=sites.index)
    if interconnection_series.isna().all():
        return pd.Series(site_total_capacity, index=timestamp_series.index, dtype="float64")
    ...
    effective_dates = interconnection_series.dt.date.where(interconnection_series.notna(), min_date)
...
def build_daily_active_capacity(
    ...
    if sites is None or sites.empty or "SolarCECkW" not in sites.columns or start_date > end_date:
        return empty
    ...
    capacity = calculate_active_capacity_for_timestamps(
        pd.Series(dates),
        sites=sites,
        default_capacity_kw=float(pd.to_numeric(sites["SolarCECkW"], errors="coerce").fillna(0.0).sum()),
    )
    return pd.DataFrame(
        {
            "Date": dates.date,
            "ActiveCapacity_kW": pd.to_numeric(capacity, errors="coerce").fillna(0.0).to_numpy(),
        }
    )
...
def _resolve_row_capacity(
    ...
    """
    timestamp_series = pd.to_datetime(pd.Series(timestamps), errors="coerce")
    fallback = pd.Series(float(fallback_capacity_kw), index=timestamp_series.index, dtype="float64")
    if daily_active_capacity is None or daily_active_capacity.empty:
        return fallback
    if "Date" not in daily_active_capacity.columns or "ActiveCapacity_kW" not in daily_active_capacity.columns:
        return fallback

    daily = daily_active_capacity.copy()
    daily["Date"] = pd.to_datetime(daily["Date"], errors="coerce").dt.date
    daily["ActiveCapacity_kW"] = pd.to_numeric(daily["ActiveCapacity_kW"], errors="coerce")
    lookup = daily.dropna(subset=["Date"]).drop_duplicates("Date", keep="last").set_index("Date")[
        "ActiveCapacity_kW"
    ]
...
def temperature_to_celsius(values: pd.Series, unit: str) -> pd.Series:
    ...
    logging.warning("Unsupported hourly temperature unit %r; leaving values as-is", unit)
    return numeric_values
...
def wind_speed_to_mps(values: pd.Series, unit: str) -> pd.Series:
    ...
    if normalized_unit in {"", "m/s", "ms", "ms-1", "meterpersecond", "meterspersecond"}:
        return numeric_values
...
def collect_reusable_hourly_weather(
    ...
    reusable = reusable[
        (reusable["date"] >= start_date)
        & (reusable["date"] <= end_date)
    ].copy()
    if reusable.empty:
        return reusable

    if expected_site_keys and "SolarSiteKey" in reusable.columns:
        site_key_mask = reusable["SolarSiteKey"].where(
            reusable["SolarSiteKey"].notna(),
            "",
        ).astype(str).isin(expected_site_keys)
...
def missing_hourly_weather_ranges(
    ...
    """
    requested_dates = [value.date() for value in pd.date_range(start_date, end_date, freq="D")]
    ...
    missing_dates = [weather_date for weather_date in requested_dates if weather_date not in covered_dates]
...
def fetch_open_meteo_hourly_weather(
    ...
    for batch_num, site_batch in enumerate(site_batches, start=1):
        ...
        for i, site_weather in enumerate(results):
            ...
            direct_normal_unit = hourly_units.get("direct_normal_irradiance", radiation_unit)
            diffuse_radiation_unit = hourly_units.get("diffuse_radiation", radiation_unit)
            global_tilted_unit = hourly_units.get("global_tilted_irradiance", radiation_unit)
            temperature_unit = hourly_units.get("temperature_2m", "\u00b0C")
            dew_point_unit = hourly_units.get("dew_point_2m", temperature_unit)
            apparent_temperature_unit = hourly_units.get("apparent_temperature", temperature_unit)
            ...
            if direct_radiation_values is not None:
                temp_df["DirectRadiation_Wm2"] = pd.to_numeric(direct_radiation_values, errors="coerce")
            ...
            if direct_normal_values is not None:
                temp_df["DirectNormalIrradiance_Wm2"] = pd.to_numeric(direct_normal_values, errors="coerce")
                if direct_normal_unit != radiation_unit:
                    temp_df["DirectNormalIrradiance_Wm2"] = (
                        hourly_irradiance_to_kwh_per_m2(direct_normal_values, direct_normal_unit) * 1000.0
                    )
            else:
            ...
            if diffuse_radiation_values is not None:
                temp_df["DiffuseRadiation_Wm2"] = pd.to_numeric(diffuse_radiation_values, errors="coerce")
                temp_df["DiffuseRadiation_kWh_per_m2"] = hourly_irradiance_to_kwh_per_m2(
            ...
            if global_tilted_values is not None:
                temp_df["GlobalTiltedIrradiance_Wm2"] = pd.to_numeric(global_tilted_values, errors="coerce")
                temp_df["GlobalTiltedIrradiance_kWh_per_m2"] = hourly_irradiance_to_kwh_per_m2(
            ...
            if temperature_values is not None:
                temp_df["Temperature2m_C"] = temperature_to_celsius(temperature_values, temperature_unit)
            else:
            ...
            temp_df["ApparentTemperature_C"] = (
                temperature_to_celsius(apparent_temperature_values, apparent_temperature_unit)
            ...
            if wind_speed_values is not None:
                temp_df["WindSpeed10m_mps"] = wind_speed_to_mps(wind_speed_values, wind_speed_unit)
            else:
...
def fetch_hourly_weather_for_date_range(
    ...
    if "SolarSiteKey" in weather.columns:
        dedupe_keys.insert(0, "SolarSiteKey")
    return weather.drop_duplicates(subset=dedupe_keys, keep="last").sort_values(dedupe_keys)
...
def build_weather_clusters(sites: pd.DataFrame, n_clusters: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for column in ["Latitude", "Longitude", "SolarCECkW"]:
        sites_with_coords[column] = pd.to_numeric(sites_with_coords[column], errors="coerce")
    ...
    for cluster_id, group in sites_with_coords.groupby("WeatherCluster", sort=True):
        ...
        distance = (
            (group["Latitude"] - target_latitude) ** 2
            + (group["Longitude"] - target_longitude) ** 2
        )
    ...
    weather_samples = pd.DataFrame(sampled_rows).sort_values("SolarSiteKey").reset_index(drop=True)
    logging.info(
        "Selected %s representative solar sites for weather sampling; largest cluster %.2f kW, total %.2f kW",
        len(weather_samples),
        weather_samples["ClusterCapacity_kW"].max() if not weather_samples.empty else 0.0,
        weather_samples["ClusterCapacity_kW"].sum() if not weather_samples.empty else 0.0,
    )
...
def aggregate_capacity_weighted_weather(weather_df: pd.DataFrame, sites: pd.DataFrame) -> pd.DataFrame:
    ...
    if "SolarSiteKey" not in weather_df.columns:
        logging.warning("Weather rows have no SolarSiteKey; using unweighted hourly weather average.")
        return aggregate_weather_to_hourly(weather_df)

    if "WeatherCluster" in sites.columns:
        capacity_by_key = sites[["WeatherCluster", "SolarCECkW"]].copy()
        capacity_by_key["WeightKey"] = pd.to_numeric(capacity_by_key["WeatherCluster"], errors="coerce")
        key_label = "weather clusters"
    else:
        capacity_by_key = sites[["SolarSiteKey", "SolarCECkW"]].copy()
        capacity_by_key["WeightKey"] = pd.to_numeric(capacity_by_key["SolarSiteKey"], errors="coerce")
        key_label = "sites"

    capacity_by_key["SolarCECkW"] = pd.to_numeric(capacity_by_key["SolarCECkW"], errors="coerce")
    capacity_by_key = (
    ...
    if capacity_by_key.empty:
        logging.warning("No positive capacity weights found for weather aggregation; using unweighted hourly average.")
        return aggregate_weather_to_hourly(weather_df)

    weather_key = pd.to_numeric(weather_df["SolarSiteKey"], errors="coerce")
    weather = pd.DataFrame(
        {
            "IntervalStartDT": pd.to_datetime(weather_df["IntervalStartDT"], errors="coerce"),
            "WeightKey": weather_key,
        }
    )
    for column in WEATHER_OUTPUT_COLUMNS:
        if column in weather_df.columns:
            weather[column] = pd.to_numeric(weather_df[column], errors="coerce").astype("float64")
        else:
    ...
    if not valid_weight.any():
        logging.warning("Weather rows did not match positive capacity weights; using unweighted hourly average.")
        return aggregate_weather_to_hourly(weather_df)
    ...
    grouped = work.groupby("IntervalStartDT", sort=True, as_index=False)[sum_columns].sum()
...
def add_physics_features(
    ...
    solar_zenith_cos = pd.Series(np.sin(elevation_radians), index=out.index).clip(lower=0.0)
    ...
    ).clip(lower=0.0, upper=1.6).fillna(0.0)
    ...
    fallback_diffuse_fraction = (0.25 + 0.55 * (1.0 - out["ClearSkyIndex"].clip(0.0, 1.0))).clip(0.20, 0.85)
    ...
    direct_wm2 = direct_wm2.where(~missing_diffuse | ~missing_direct, (ghi_wm2 - diffuse_wm2).clip(lower=0.0))
    ...
    incidence_cos = (
        np.cos(solar_zenith_radians) * math.cos(tilt_radians)
        + np.sin(solar_zenith_radians)
        * math.sin(tilt_radians)
        * np.cos(solar_azimuth_radians - array_azimuth_radians)
    )
    out["IncidenceAngleCos"] = pd.Series(incidence_cos, index=out.index).clip(lower=0.0, upper=1.0)
    ...
    out["DirectPOA_Wm2"] = direct_poa_from_dni.where(dni_wm2.notna(), direct_poa_from_horizontal)
    out["DiffusePOA_Wm2"] = (diffuse_wm2 * (1.0 + math.cos(tilt_radians)) / 2.0).clip(lower=0.0)
    ground_reflected_wm2 = ghi_wm2 * GROUND_ALBEDO * (1.0 - math.cos(tilt_radians)) / 2.0
    modeled_poa_wm2 = (
    ...
    out["CellTemperature_C"] = cell_temperature.fillna(out["Temperature2m_C"]).fillna(25.0)
...
def add_performance_features(
    ...
    out["GHI_kWh_per_m2"] = out["GHI_kWh_per_m2"].fillna(out["WeatherGHI_Wm2"] / 1000.0).clip(lower=0)
    ...
    out["DirectRadiation_kWh_per_m2"] = out["DirectRadiation_kWh_per_m2"].fillna(
        out["DirectRadiation_Wm2"] / 1000.0
    ).clip(lower=0)
    out["DirectRadiation_Wm2"] = out["DirectRadiation_Wm2"].fillna(out["DirectRadiation_kWh_per_m2"] * 1000.0)
    out["DirectNormalIrradiance_Wm2"] = out["DirectNormalIrradiance_Wm2"].clip(lower=0.0)

    out["DiffuseRadiation_Wm2"] = out["DiffuseRadiation_Wm2"].clip(lower=0)
    out["DiffuseRadiation_kWh_per_m2"] = out["DiffuseRadiation_kWh_per_m2"].fillna(
        out["DiffuseRadiation_Wm2"] / 1000.0
    ).clip(lower=0)
    out["DiffuseRadiation_Wm2"] = out["DiffuseRadiation_Wm2"].fillna(out["DiffuseRadiation_kWh_per_m2"] * 1000.0)

    out["GlobalTiltedIrradiance_Wm2"] = out["GlobalTiltedIrradiance_Wm2"].clip(lower=0.0)
    out["GlobalTiltedIrradiance_kWh_per_m2"] = out["GlobalTiltedIrradiance_kWh_per_m2"].fillna(
        out["GlobalTiltedIrradiance_Wm2"] / 1000.0
    ).clip(lower=0.0)
    out["GlobalTiltedIrradiance_Wm2"] = out["GlobalTiltedIrradiance_Wm2"].fillna(
        out["GlobalTiltedIrradiance_kWh_per_m2"] * 1000.0
    )

    out["Temperature2m_C"] = out["Temperature2m_C"].fillna(out["Temperature2m_C"].median()).fillna(25.0)
    out["Temperature_C"] = out["Temperature_C"].fillna(out["Temperature2m_C"])
    out["RelativeHumidity2mPct"] = out["RelativeHumidity2mPct"].clip(lower=0, upper=100).fillna(50.0)
    out["DewPoint2m_C"] = out["DewPoint2m_C"].fillna(out["Temperature2m_C"])
    out["ApparentTemperature_C"] = out["ApparentTemperature_C"].fillna(out["Temperature2m_C"])
    out["SurfacePressure_hPa"] = out["SurfacePressure_hPa"].fillna(out["SurfacePressure_hPa"].median()).fillna(1013.25)
    out["WindSpeed10m_mps"] = out["WindSpeed10m_mps"].clip(lower=0).fillna(0.0)
    out["WindSpeed_ms"] = out["WindSpeed_ms"].clip(lower=0).fillna(out["WindSpeed10m_mps"])
    out["WindDirection10mDeg"] = out["WindDirection10mDeg"].mod(360.0).fillna(0.0)
    out["WindGusts10m_mps"] = out["WindGusts10m_mps"].clip(lower=0.0).fillna(out["WindSpeed10m_mps"])
    out["WindGusts10m_mps"] = out["WindGusts10m_mps"].where(
        out["WindGusts10m_mps"] >= out["WindSpeed10m_mps"],
        out["WindSpeed10m_mps"],
    )
    out["VapourPressureDeficit_kPa"] = out["VapourPressureDeficit_kPa"].clip(lower=0.0).fillna(0.0)
    out["Precipitation_mm"] = out["Precipitation_mm"].clip(lower=0.0).fillna(0.0)
    out["WeatherCode"] = out["WeatherCode"].fillna(0.0)
    out["SunshineDurationSec"] = out["SunshineDurationSec"].clip(lower=0.0, upper=3600.0).fillna(0.0)
    ...
    out["IsDay"] = out["IsDay"].fillna((out["SolarElevationDeg"] > 0).astype(float)).clip(lower=0.0, upper=1.0)
    missing_gti = out["GlobalTiltedIrradiance_Wm2"].isna()
    out.loc[missing_gti, "GlobalTiltedIrradiance_Wm2"] = out.loc[missing_gti, "PlaneOfArrayIrradiance_Wm2"]
...
def add_solar_actual_quality_flags(
    ...
    ratio = (actual_kwh / expected_kwh.where(expected_kwh > 0)).replace([np.inf, -np.inf], np.nan)
    ...
    typical_read_count = float(positive_read_count.quantile(0.90)) if not positive_read_count.empty else np.nan
    ...
    if actual_read_count_col and actual_read_count_col in out.columns:
        severe_physical = physical_suspicious_hour & ratio.le(actual_to_expected_ratio_threshold * 0.50)
    ...
    out["SolarBacktestExcluded"] = _actual_quality_exclusion_mask(out) | quality_excluded.fillna(False).astype(bool)
...
def train_performance_model(
    ...
    training_data["ModeledAvailable_kWh"] = available_per_kw * training_data["ActiveCapacity_kW"]
    ...
    if training_data.empty:
        ...
        return PerformanceModel(None, fallback_ratio, PERFORMANCE_FEATURE_COLUMNS, performance_ratio_upper_bound)
    ...
    learned_fallback = float(np.clip(learned_fallback, 0.0, performance_ratio_upper_bound))
    ...
    if len(training_data) < min_training_rows:
        ...
        return PerformanceModel(None, learned_fallback, PERFORMANCE_FEATURE_COLUMNS, performance_ratio_upper_bound)
    ...
    fitted = pd.Series(model.predict(X), index=training_data.index).clip(0.0, performance_ratio_upper_bound)
    ...
    return PerformanceModel(model, learned_fallback, PERFORMANCE_FEATURE_COLUMNS, performance_ratio_upper_bound)
...
def predict_performance_ratio(model: PerformanceModel, feature_df: pd.DataFrame) -> pd.Series:
    ...
    ratio = pd.Series(
        model.estimator.predict(feature_df.reindex(columns=model.feature_columns).fillna(0.0)),
        index=feature_df.index,
    )
    return ratio.clip(lower=0.0, upper=model.ratio_upper_bound).fillna(model.fallback_ratio)
...
def add_residual_calibration_features(
    ...
    out["PerformanceRatio"] = out["PerformanceRatio"].fillna(0.0).clip(
        lower=0.0,
...
def filter_residual_calibration_training_quality(
    ...
    daylight = work[work["Forecast_kWh"].fillna(0.0) >= min_training_forecast_kwh].copy()
    if daylight.empty:
        return training_data

    daylight["CalibrationDate"] = daylight["IntervalStartDT"].dt.date
    daily = (
        daylight.groupby("CalibrationDate", as_index=False)
        .agg(
    ...
    daily["RowCoveragePct"] = daily["ObservedActualHours"] / daily["ExpectedDaylightHours"].where(
        daily["ExpectedDaylightHours"] > 0
    )
    daily["ActualForecastRatio"] = daily["Actual_kWh"] / daily["Forecast_kWh"].where(daily["Forecast_kWh"] > 0)
...
def build_regime_key_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    index = df.index
    clear_sky_index = pd.to_numeric(df.get("ClearSkyIndex", pd.Series(0.0, index=index)), errors="coerce")
    cloud_cover = pd.to_numeric(df.get("CloudCoverPct", pd.Series(0.0, index=index)), errors="coerce")
    solar_elevation = pd.to_numeric(df.get("SolarElevationDeg", pd.Series(90.0, index=index)), errors="coerce")
    ...
    csi_bucket = pd.cut(
        ...
        include_lowest=True,
    ).astype("object").fillna(REGIME_CLEAR_SKY_LABELS[0]).astype(str)
    cloud_bucket = pd.cut(
        cloud_cover.clip(lower=0.0, upper=100.0).fillna(0.0),
        bins=REGIME_CLOUD_BINS,
        labels=REGIME_CLOUD_LABELS,
        include_lowest=True,
    ).astype("object").fillna(REGIME_CLOUD_LABELS[0]).astype(str)
    elevation_bucket = pd.cut(
        solar_elevation.clip(lower=-90.0, upper=90.0).fillna(90.0),
        bins=REGIME_ELEVATION_BINS,
        labels=REGIME_ELEVATION_LABELS,
        include_lowest=True,
    ).astype("object").fillna(REGIME_ELEVATION_LABELS[-1]).astype(str)

    return pd.DataFrame(
        {
            "RegimeKey": "regime|" + csi_bucket + "|" + cloud_bucket + "|" + elevation_bucket,
...
def build_regime_factor_map(
    ...
    work = work[
        (work["Actual_kWh"] >= 0)
        & (work["PreliminaryForecast_kWh"] > 0)
    ].copy()
    if work.empty:
        return {}
    ...
    for group_column in ["RegimeKey", "RegimeFallbackKey"]:
        grouped = (
            work.groupby(group_column, as_index=False)
            .agg(
        ...
        for _, row in grouped.iterrows():
            raw_factor = (row["Actual_kWh"] + prior_kwh) / (row["Forecast_kWh"] + prior_kwh)
            factors[str(row[group_column])] = float(np.clip(raw_factor, lower_bound, upper_bound))
...
def prune_regime_factors_by_holdout(
    ...
    for key_column in ["RegimeKey", "RegimeFallbackKey"]:
        for key, factor in candidate_factors.items():
            ...
            if len(subset) < REGIME_HOLDOUT_MIN_ROWS or forecast_mwh < REGIME_HOLDOUT_MIN_FORECAST_MWH:
                continue
...
def calculate_peak_capture(actual_power_kw: pd.Series, forecast_power_kw: pd.Series) -> float:
    """
...
def active_capacity_for_peak_calibration(df: pd.DataFrame, total_capacity_kw: float) -> pd.Series:
    """
...
def actual_power_kw_for_peak_calibration(df: pd.DataFrame) -> pd.Series:
    ...
    if "Actual_kW" in df.columns:
        ...
        if actual_kw.notna().any():
            return actual_kw
    return pd.to_numeric(df.get("Actual_kWh", pd.Series(np.nan, index=df.index)), errors="coerce")
...
def build_peak_calibration_params(
    ...
    work["ActiveCapacity_kW"] = active_capacity_for_peak_calibration(work, total_capacity_kw)
    work = work.dropna(subset=["ActualPower_kW", "PrePeakForecast_kWh", "PrePeakForecastPower_kW", "ActiveCapacity_kW"])
    work = work[
    ...
    work["PrePeakCapacityFactor"] = work["PrePeakForecastPower_kW"] / work["ActiveCapacity_kW"]
    ...
    if pd.isna(actual_peak_kw) or actual_peak_kw <= 0 or pd.isna(forecast_peak_kw) or forecast_peak_kw <= 0:
        return 1.0, None, {}

    raw_factor = target_capture * actual_peak_kw / forecast_peak_kw
    factor = float(np.clip(raw_factor, lower_bound, 1.0))
    peak_capture = calculate_peak_capture(work["ActualPower_kW"], work["PrePeakForecastPower_kW"])
...
def lookup_peak_calibration_factor(
    ...
    forecast_power_kw = pd.to_numeric(forecast_power_kw.reindex(feature_df.index), errors="coerce")
...
def tune_peak_calibration_from_hourly_backtest(
    ...
    work["ActualPower_kW"] = (
        pd.to_numeric(work.get("Actual_MW", pd.Series(np.nan, index=work.index)), errors="coerce") * 1000.0
    )
    if work["ActualPower_kW"].isna().all():
        work["ActualPower_kW"] = actual_power_kw_for_peak_calibration(work)
    work["ForecastPower_kW"] = (
        pd.to_numeric(work.get("Forecast_MW", pd.Series(np.nan, index=work.index)), errors="coerce") * 1000.0
    )
    if work["ForecastPower_kW"].isna().all():
        work["ForecastPower_kW"] = pd.to_numeric(work.get("Forecast_kWh"), errors="coerce")
    work["Forecast_kWh"] = pd.to_numeric(
        work.get("Forecast_kWh", pd.Series(np.nan, index=work.index)),
        errors="coerce",
    )
    work["ActiveCapacity_kW"] = active_capacity_for_peak_calibration(work, total_capacity_kw)
    work = work.dropna(subset=["ActualPower_kW", "ForecastPower_kW", "Forecast_kWh", "ActiveCapacity_kW"])
    work = work[
        (work["ActualPower_kW"] >= 0)
    ...
    peak_capture = calculate_peak_capture(work["ActualPower_kW"], work["ForecastPower_kW"])
    if pd.isna(peak_capture) or peak_capture <= target_capture + PEAK_CAPTURE_TOLERANCE:
        return False

    work["ForecastCapacityFactor"] = work["ForecastPower_kW"] / work["ActiveCapacity_kW"]
...
def train_residual_calibration_model(
    ...
    training_data["IntervalStartDT"] = pd.to_datetime(training_data["IntervalStartDT"], errors="coerce")
    training_data["Actual_kWh"] = pd.to_numeric(training_data["Actual_kWh"], errors="coerce")
    training_data["Forecast_kWh"] = pd.to_numeric(training_data["Forecast_kWh"], errors="coerce")
    ...
    if use_quality_filter:
        ...
        if training_data.empty:
            logging.info("Residual calibration skipped; quality filter removed all daylight training rows.")
            return model
    ...
    sample_weight = None
    if use_energy_weighting:
        sample_weight = training_data["Forecast_kWh"].clip(lower=min_training_forecast_kwh) ** energy_weight_power
    ...
    holdout_rows = max(min_validation_rows, holdout_rows) if validation_fraction > 0 else 0
    ...
    if holdout_rows >= min_validation_rows:
        ...
        validation_weights = sample_weight.loc[fit_index] if sample_weight is not None else None
        validation_estimator.fit(X.loc[fit_index], y.loc[fit_index], sample_weight=validation_weights)
        ...
        if use_seasonal_calibration:
            validation_seasonal_factors, validation_default_factor = build_seasonal_calibration_factors(
            ...
            if validation_seasonal_factors:
                fit_month_factor = training_data.loc[fit_index, "IntervalStartDT"].dt.month.map(
                    validation_seasonal_factors
                ).fillna(validation_default_factor)
                validation_month_factor = validation_subset["IntervalStartDT"].dt.month.map(
                    validation_seasonal_factors
                ).fillna(validation_default_factor)
        ...
        validation_base_wmape = (
            (validation_subset["Forecast_kWh"] - validation_subset["Actual_kWh"]).abs().sum()
            / validation_actual_kwh
            if validation_actual_kwh > 0
            else np.nan
        )
        validation_pre_regime_kwh = validation_subset["Forecast_kWh"] * validation_factor
        ...
        if use_regime_calibration:
            validation_regime_factors = build_regime_factor_map(
                ...
                preliminary_forecast_kwh=training_data.loc[fit_index, "Forecast_kWh"] * fit_factor,
            ...
            validation_calibrated_kwh = validation_pre_regime_kwh * validation_regime_factor
        validation_calibrated_wmape = (
            (validation_calibrated_kwh - validation_subset["Actual_kWh"]).abs().sum() / validation_actual_kwh
            if validation_actual_kwh > 0
            else np.nan
        )
        ...
        if use_peak_calibration:
            validation_peak_factor, validation_peak_threshold_cf, validation_peak_diag = build_peak_calibration_params(
                calibration_data=training_data.loc[fit_index],
                pre_peak_forecast_kwh=training_data.loc[fit_index, "Forecast_kWh"] * fit_factor,
                total_capacity_kw=total_capacity_kw,
            ...
            validation_peak_kwh = validation_calibrated_kwh * validation_peak_factor_series
        ...
        logging.info(
            ...
            validation_base_wmape * 100 if pd.notna(validation_base_wmape) else float("nan"),
            validation_pre_regime_wmape * 100 if pd.notna(validation_pre_regime_wmape) else float("nan"),
            validation_calibrated_wmape * 100 if pd.notna(validation_calibrated_wmape) else float("nan"),
        ...
        if use_peak_calibration:
            logging.info(
                ...
                validation_pre_peak_capture * 100 if pd.notna(validation_pre_peak_capture) else float("nan"),
                validation_peak_capture * 100 if pd.notna(validation_peak_capture) else float("nan"),
                validation_pre_peak_wmape * 100 if pd.notna(validation_pre_peak_wmape) else float("nan"),
                validation_peak_wmape * 100 if pd.notna(validation_peak_wmape) else float("nan"),
                validation_peak_factor,
                validation_peak_threshold_cf if validation_peak_threshold_cf is not None else float("nan"),
        ...
        if (
            ...
            and validation_calibrated_wmape > validation_pre_regime_wmape + REGIME_HOLDOUT_WMAPE_TOLERANCE
        ):
        ...
        if (
            ...
            pre_peak_excess = max(0.0, validation_pre_peak_capture - peak_target_capture) if pd.notna(
                validation_pre_peak_capture
            ) else np.nan
            post_peak_excess = max(0.0, validation_peak_capture - peak_target_capture) if pd.notna(
                validation_peak_capture
            ) else np.nan
            if (
                validation_peak_wmape > validation_pre_peak_wmape + PEAK_HOLDOUT_WMAPE_TOLERANCE
                or (
    ...
    if use_regime_calibration and not disable_regime_due_holdout:
        ...
        fitted_regime_factor = lookup_regime_calibration_factor_from_map(training_data, regime_factors)
    ...
    if use_peak_calibration and not disable_peak_due_holdout:
        peak_calibration_factor, peak_calibration_threshold_cf, peak_diag = build_peak_calibration_params(
            calibration_data=training_data,
        ...
        if peak_calibration_factor < 1.0 and peak_calibration_threshold_cf is not None:
            ...
            logging.info(
                ...
                post_peak_capture * 100 if pd.notna(post_peak_capture) else float("nan"),
            )
...
def apply_residual_calibration(
    ...
    out.loc[active_mask, "Forecast_kWh"] = (
        out.loc[active_mask, "Forecast_kWh"] * out.loc[active_mask, "PeakCalibrationFactor"]
    )
    out.loc[active_mask, "Forecast_kW"] = (
        out.loc[active_mask, "Forecast_kW"] * out.loc[active_mask, "PeakCalibrationFactor"]
    )
    out["TotalCalibrationFactor"] = (
        out["ResidualCalibrationFactor"]
        * out["SeasonalCalibrationFactor"]
...
def build_interval_forecast(
    ...
    if sites is not None and not isinstance(sites, pd.DataFrame) and isinstance(longitude, str):
    ...
    forecast_df["Hourly_kWh"] = (
        available_per_kw * forecast_df["ActiveCapacity_kW"] * forecast_df["PerformanceRatio"]
    )
    ...
    if peak_hourly_kwh_quantile is not None and not interval_forecast.empty:
        peak_threshold = float(interval_forecast["Hourly_kWh"].quantile(float(peak_hourly_kwh_quantile)))
        if pd.notna(peak_threshold) and peak_threshold > 0:
            peak_mask = (
                interval_forecast["Hourly_kWh"].ge(peak_threshold)
            ...
            if bool(peak_mask.any()):
                ...
                target = interval_forecast.loc[peak_mask, "minute"].map(peak_shape).fillna(0.25)
                interval_forecast.loc[peak_mask, "IntraHourCoefficient"] = 0.6 * base + 0.4 * target
                affected_hours = interval_forecast.loc[peak_mask, "IntervalStartDT"].drop_duplicates()
                affected_mask = interval_forecast["IntervalStartDT"].isin(affected_hours)
                coefficient_sum = interval_forecast.loc[affected_mask].groupby("IntervalStartDT")[
                    "IntraHourCoefficient"
                ].transform("sum")
                interval_forecast.loc[affected_mask, "IntraHourCoefficient"] = (
                    interval_forecast.loc[affected_mask, "IntraHourCoefficient"] / coefficient_sum.replace(0, np.nan)
                ).fillna(0.25)
...
def apply_same_day_actual_correction(
    ...
    if "CloudCoverPct" in comparison.columns and "CloudCoverPct" in future.columns:
        ...
        similarity_components.append((1.0 - (future_cloud - observed_cloud).abs() / 100.0).clip(0.0, 1.0))
    if "ClearSkyIndex" in comparison.columns and "ClearSkyIndex" in future.columns:
        ...
        similarity_components.append((1.0 - (future_clear_sky_index - observed_clear_sky_index).abs()).clip(0.0, 1.0))
    ...
    effective_factor = 1.0 + (correction_factor - 1.0) * lead_weight * weather_similarity
...
def resample_interval_forecast_to_hourly(interval_forecast: pd.DataFrame, total_capacity_kw: float) -> pd.DataFrame:
    ...
    if "CustomerSegment" in interval_forecast.columns:
        ...
        hourly_forecast = interval_forecast.set_index("IntervalStartDT").resample("h").agg(**hourly_aggregations)
    ...
    capacity_denominator = pd.to_numeric(hourly_forecast["ActiveCapacity_kW"], errors="coerce")
    if total_capacity_kw > 0:
        capacity_denominator = capacity_denominator.where(capacity_denominator > 0, total_capacity_kw)
    else:
...
def build_hourly_backtest(
    ...
    if "CustomerSegment" in actual_hourly.columns and "CustomerSegment" in forecast_hourly.columns:
...
def calculate_backtest_summary(backtest_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if bool(excluded_mask.any()):
        ...
        base_col = "BaseForecast_kWh" if "BaseForecast_kWh" in raw_backtest_df.columns else "Forecast_kWh"
    ...
    if "APE" not in backtest_df.columns:
        ...
        positive_actual_mask = pd.to_numeric(backtest_df["Actual_kWh"], errors="coerce") > 0
...
def _solar_metric_row(
    ...
    base_wmape = base_error_kwh.abs().sum() / actual_kwh_sum if actual_kwh_sum > 0 else np.nan
    wmape_improvement = (
        (base_wmape - wmape) / base_wmape
        if pd.notna(base_wmape) and base_wmape > 0 and pd.notna(wmape)
        else np.nan
    ...
    row.update(
        {
            ...
            "BiasPct": (forecast_kwh_sum - actual_kwh_sum) / actual_kwh_sum if actual_kwh_sum > 0 else np.nan,
            "BaseBiasPct": (
...
def build_solar_backtest_top_errors(
    ...
    active_mask = (
        work["Actual_MW"].fillna(0.0).gt(daylight_threshold_mw)
        | work["Forecast_MW"].fillna(0.0).gt(daylight_threshold_mw)
    )
...
def build_solar_temporal_holdout_backtest(
    ...
    if actual_quality_filter_enabled and not holdout_hourly.empty:
        holdout_hourly["ActualQualityExpected_kWh"] = holdout_hourly[["Forecast_kWh", "BaseForecast_kWh"]].max(axis=1)
...
def run_forecaster(args: argparse.Namespace) -> None:
    ...
    if not (0 < args.performance_ratio <= args.performance_ratio_upper_bound):
        raise ValueError("--performance-ratio must be greater than 0 and less than or equal to the ratio upper bound.")
    ...
    if args.same_day_correction_half_life_hours <= 0:
        raise ValueError("--same-day-correction-half-life-hours must be greater than 0.")
    if not (0 <= args.same_day_correction_weather_similarity_floor <= 1):
        raise ValueError("--same-day-correction-weather-similarity-floor must be between 0 and 1.")
    ...
    if args.residual_calibration_min_validation_rows <= 0:
        raise ValueError("--residual-calibration-min-validation-rows must be greater than 0.")
    if not (0 <= args.residual_calibration_validation_fraction < 0.5):
        raise ValueError("--residual-calibration-validation-fraction must be at least 0 and less than 0.5.")
    ...
    if args.residual_calibration_energy_weight_power < 0:
        raise ValueError("--residual-calibration-energy-weight-power must be greater than or equal to 0.")
    customer_segments = parse_customer_segments(args.customer_segments)
    if args.segment_forecasts and not customer_segments:
        raise ValueError("--customer-segments must include at least one segment when --segment-forecasts is enabled.")
    ...
    if not (0 <= args.calibration_min_daylight_row_coverage <= 1):
        raise ValueError("--calibration-min-daylight-row-coverage must be between 0 and 1.")
    if args.calibration_min_day_actual_forecast_ratio < 0:
        raise ValueError("--calibration-min-day-actual-forecast-ratio must be greater than or equal to 0.")
    if args.calibration_min_day_forecast_mwh < 0:
        raise ValueError("--calibration-min-day-forecast-mwh must be greater than or equal to 0.")
    ...
    if args.regime_calibration_min_forecast_mwh < 0:
        raise ValueError("--regime-calibration-min-forecast-mwh must be greater than or equal to 0.")
    if args.regime_calibration_prior_mwh < 0:
        raise ValueError("--regime-calibration-prior-mwh must be greater than or equal to 0.")
    if not (0 < args.regime_calibration_lower_bound <= args.regime_calibration_upper_bound):
        raise ValueError(
    ...
    if not (0 < args.peak_calibration_quantile < 1):
        raise ValueError("--peak-calibration-quantile must be greater than 0 and less than 1.")
    ...
    if args.peak_calibration_min_forecast_mwh < 0:
        raise ValueError("--peak-calibration-min-forecast-mwh must be greater than or equal to 0.")
    if not (0 < args.peak_calibration_lower_bound <= 1):
        raise ValueError("--peak-calibration-lower-bound must be greater than 0 and less than or equal to 1.")
    ...
    try:
        ...
        if args.production_source == "rec-parquet":
            ...
            if args.segment_forecasts:
                rec_segment_hourly = resample_actual_export_to_hourly(rec_segment_interval_df)
                rec_segment_hourly[
                    ["CustomerSegment", "IntervalStartDT", "HE", "Export_MW", "Export_kWh", "Export_kW"]
            ...
            if args.use_capacity_weighted_weather:
                ...
                if sites_with_coords.empty:
                    ...
                    weather_location_budget = args.max_weather_api_calls * args.weather_locations_per_request
                    requested_weather_samples = min(args.weather_clusters, len(sites_with_coords))
                    weather_sample_count = min(requested_weather_samples, weather_location_budget)
                    ...
                    logging.info(
                        ...
                        len(sites_with_coords),
                        int(math.ceil(weather_sample_count / args.weather_locations_per_request)),
                    )
                    sites, weather_sites_df = build_weather_clusters(sites, n_clusters=weather_sample_count)
                else:
                    logging.info("Weather site sampling disabled; using single representative Roseville weather.")
                    args.use_capacity_weighted_weather = False
                    weather_sites_df = build_system_weather_site(args.latitude, args.longitude)
            else:
            ...
            if args.backtest:
                ...
                diagnostics_output = getattr(args, "solar_backtest_diagnostics_output", None)
                ...
                top_errors_output = getattr(args, "solar_backtest_top_errors_output", None)
                if top_errors_output:
                    top_errors = build_solar_backtest_top_errors(
                        backtest_hourly,
                        top_n=getattr(args, "solar_backtest_top_error_count", DEFAULT_SOLAR_BACKTEST_TOP_ERROR_COUNT),
                        daylight_threshold_mw=getattr(
                            args,
                            "solar_backtest_daylight_threshold_mw",
                            DEFAULT_SOLAR_BACKTEST_DAYLIGHT_THRESHOLD_MW,
                ...
                if holdout_output:
                    holdout_scorecard, holdout_hourly = build_solar_temporal_holdout_backtest(
                        ...
                        holdout_days=getattr(args, "solar_backtest_holdout_days", DEFAULT_SOLAR_BACKTEST_HOLDOUT_DAYS),
                    ...
                    holdout_hourly_output = getattr(args, "solar_backtest_holdout_hourly_output", None)
                    if holdout_hourly_output and not holdout_hourly.empty:
                        Path(holdout_hourly_output).parent.mkdir(parents=True, exist_ok=True)
        ...
        if (
            ...
            logging.info(
                ...
                ", ".join(CUSTOMER_SEGMENT_LABELS.get(segment, segment) for segment in customer_segments),
            )
            ...
            for segment in customer_segments:
                ...
                segment_capacity_kw = float(pd.to_numeric(segment_sites["SolarCECkW"], errors="coerce").sum())

                if segment_sites.empty or segment_rec_intervals.empty or segment_capacity_kw <= 0:
                    logging.warning(
                ...
                if args.backtest and not segment_calibration_interval_forecast.empty:
                    if args.residual_calibration:
                        ...
                        segment_backtest_interval_forecast["ForecastSource"] = "backtest_calibrated"
                    else:
                        segment_backtest_interval_forecast = segment_calibration_interval_forecast
                    ...
                    if args.residual_calibration and args.peak_calibration:
                        ...
                        if peak_tuned:
                            ...
                            segment_backtest_hourly["CustomerSegment"] = segment
                    segment_summary = calculate_backtest_summary(segment_backtest_hourly)
                ...
                if (
                    ...
                    segment_inferred_forecast = segment_inferred_forecast[
                        (segment_inferred_forecast["IntervalStartDT"] >= inferred_start_timestamp)
                        & (segment_inferred_forecast["IntervalStartDT"] <= inferred_end_timestamp)
                    ].copy()
                    segment_forecast_parts.append(add_hour_ending_column(segment_inferred_forecast))
                ...
                segment_forecast_parts.append(add_hour_ending_column(segment_interval_forecast))
                segment_full_interval_forecast = (
                    pd.concat(segment_forecast_parts, ignore_index=True)
                    .drop_duplicates(subset=["CustomerSegment", "IntervalStartDT"], keep="last")
            ...
            if segment_interval_outputs:
                segmented_interval_forecast = pd.concat(segment_interval_outputs, ignore_index=True)
                ...
                logging.info("Segmented 15-minute forecast saved to %s", args.segment_output_15min)
                logging.info("Segmented hourly forecast saved to %s", args.segment_output_hourly)

            if segment_hourly_backtests:
                segmented_backtest_hourly = pd.concat(segment_hourly_backtests, ignore_index=True)
                ...
                logging.info("Segmented backtest saved to %s", args.segment_backtest_hourly_output)
            ...
            if segment_shape_outputs:
                ...
                logging.info("Segmented load shapes saved to %s", args.segment_load_shape_output)
