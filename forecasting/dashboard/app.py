...
def _make_forecast_graph(df: pd.DataFrame, weather_variable: str | None):
    fig = go.Figure()
    has_previous = (
        "Previous_Forecast_MWH" in df.columns
        and pd.to_numeric(df["Previous_Forecast_MWH"], errors="coerce").notna().any()
    )
    has_previous_miss = (
        "Previous_Forecast_Miss_MWH" in df.columns
        and pd.to_numeric(df["Previous_Forecast_Miss_MWH"], errors="coerce")
        .notna()
        .any()
    )
    has_prior_run = (
        "Prior_Run_Forecast_MWH" in df.columns
        and pd.to_numeric(df["Prior_Run_Forecast_MWH"], errors="coerce").notna().any()
    )
    has_forecast_change = (
        "Forecast_Change_From_Prior_Run_MWH" in df.columns
        and pd.to_numeric(df["Forecast_Change_From_Prior_Run_MWH"], errors="coerce")
        .notna()
        .any()
    )
    has_prior_temp = (
        "Prior_Run_Temperature_F" in df.columns
        and pd.to_numeric(df["Prior_Run_Temperature_F"], errors="coerce").notna().any()
    )
    has_weather = bool(
        weather_variable
    ...
    if has_weather:
        fig.add_trace(
            go.Scatter(
                x=df["DT"],
                y=df[weather_variable],
                mode="lines",
                name=weather_variable,
                yaxis="y2",
                line=dict(color=BRAND_GOLD, width=1.8, dash="dash"),
            )
        )
...
def _attach_prior_run_comparison(
    ...
    if (
        previous_weather_df is not None
        and not previous_weather_df.empty
        and "DT" in previous_weather_df.columns
    ):
        ...
        if temp_col is not None:
            ...
            prev_wx["__DT_KEY"] = pd.to_datetime(
                prev_wx["DT"], errors="coerce", utc=True
            )
            prev_wx.rename(columns={temp_col: "Prior_Run_Temperature_F"}, inplace=True)
            prev_wx["Prior_Run_Temperature_F"] = pd.to_numeric(
                prev_wx["Prior_Run_Temperature_F"], errors="coerce"
            )
            prev_wx = prev_wx.dropna(subset=["__DT_KEY"]).drop_duplicates(
                subset=["__DT_KEY"], keep="last"
            )
            out = out.merge(
                prev_wx[["__DT_KEY", "Prior_Run_Temperature_F"]],
                on="__DT_KEY",
                how="left",
            )
...
def _cards(
    ...
    cards = [
        ...
        _metric_card(
            ...
            (
                pd.to_datetime(peak_dt).strftime("%m/%d %H:%M")
    ...
    if not fut.empty and "DT" in fut.columns:
        ...
        cards.append(
            _metric_card(
                "First Forecast Hour",
                pd.to_datetime(first_forecast_dt).strftime("%m/%d %H:%M"),
...
from forecasting.dashboard.dashboard_app import (
    create_dashboard_app as create_dashboard_app,
)
