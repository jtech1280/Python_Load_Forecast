...
def _has_explicit_timezone(values: pd.Series) -> bool:
    ...
    return (
        non_null.astype(str)
        .str.strip()
        .str.contains(r"(?:[zZ]|[+-]\d{2}:?\d{2})$", regex=True, na=False)
        .any()
    )
...
def _metric_card(title: str, value: str, subtitle: str = ""):
    return html.Div(
        [
            ...
            (
                html.Div(subtitle, style={"fontSize": "11px", "color": BRAND_GREEN})
                if subtitle
                else None
            ),
        ],
        style={
            ...
            "padding": "9px",
            "marginBottom": "8px",
            "boxShadow": "0 1px 3px rgba(0, 87, 184, 0.12)",
        },
    )


def _slice_display(
    display_df: pd.DataFrame, horizon_days: int, history_hours: int
) -> pd.DataFrame:
    ...
    return display_df[
        (display_df["DT"] >= start_dt) & (display_df["DT"] <= end_dt)
    ].copy()
...
def _future_only(display_df: pd.DataFrame, horizon_days: int) -> pd.DataFrame:
    ...
    end_dt = latest_hist_dt + pd.Timedelta(days=int(horizon_days or 16))
    return display_df[
        (display_df["DT"] > latest_hist_dt) & (display_df["DT"] <= end_dt)
    ].copy()


def _display_confidence_band(
    df: pd.DataFrame,
) -> tuple[pd.Series, pd.Series, pd.Series]:
    base = pd.to_numeric(df.get("Forecast"), errors="coerce")
    raw_band = (
        pd.to_numeric(df.get("Band"), errors="coerce")
        if "Band" in df.columns
        else pd.Series(np.nan, index=df.index)
    )
    if raw_band.notna().sum() == 0 and {"Upper_Band", "Lower_Band"}.issubset(
        df.columns
    ):
    ...
    horizon = df.get("Operational_Horizon_Label", pd.Series("", index=df.index)).astype(
        str
    )
    ...
    risk_mult = pd.to_numeric(
        df.get("Weather_Input_Risk_Multiplier", pd.Series(1.0, index=df.index)),
        errors="coerce",
    ).fillna(1.0)
    high_risk = risk_mult.gt(1.0) | horizon.str.contains(
        "low_confidence", case=False, na=False
    )
    global_cap = max(12.0, float(raw_band.loc[forecast_mask].quantile(0.90)) * 0.95)
    high_risk_cap = max(
        global_cap, float(raw_band.loc[forecast_mask].quantile(0.95)) * 0.90
    )
    load_cap = base.abs() * 0.18
    cap = np.minimum(load_cap.where(load_cap.notna(), global_cap), global_cap)
    cap.loc[high_risk] = np.minimum(
        load_cap.loc[high_risk].where(load_cap.loc[high_risk].notna(), high_risk_cap),
        high_risk_cap,
    )
...
# --- Forecast series registry -------------------------------------------------
# Every plottable line on the forecast chart is declared here so the operator can
# overlay any individual component model, intermediate correction stage, weather
# scenario, or the published output via the series selector. The selector only
# offers series whose columns are actually present in the loaded data, and the
# graph builder skips anything missing, so this stays correct as columns evolve
...
FORECAST_SERIES_REGISTRY = [
    {
        ...
        "col": "Forecast",
        "line": dict(color=BRAND_BLUE, width=2.8),
        "group": "Published",
    },
    {
        "value": "Calibrated",
        "label": "Calibrated Forecast",
        "col": "Calibrated_Forecast_MWH",
        "line": dict(color="#0057B8", width=2.0, dash="dash"),
        "group": "Published",
    },
    {
        "value": "Stage_Selected",
        "label": "Stage-Selected",
        "col": "Stage_Selected_Forecast_MWH",
        "line": dict(color="#0091D5", width=1.8, dash="dash"),
        "group": "Published",
    },
    {
        "value": "Actual",
        "label": "Actual",
        "col": "Actual",
        "line": dict(color=BRAND_RED, width=2.1),
        "group": "Reference",
    },
    {
        "value": "Raw",
        "label": "Raw XGB+LGB",
        "col": "Raw_Forecast_MWH",
        "line": dict(color=BRAND_PURPLE, width=1.5, dash="dot"),
    ...
    },
    {
        ...
        "label": "XGB",
        "col": "XGB_Pred_MWH",
        "line": dict(color="#7B2CBF", width=1.4, dash="dot"),
        "group": "Component models",
    },
    {
        "value": "LGB",
        "label": "LGB",
        "col": "LGB_Pred_MWH",
        "line": dict(color="#17A2B8", width=1.4, dash="dot"),
        "group": "Component models",
    },
    {
        "value": "CatBoost",
        "label": "CatBoost",
        "col": "CatBoost_Pred_MWH",
    ...
    },
    {
        "value": "Prophet",
        "label": "Prophet",
        "col": "Prophet_Pred_MWH",
        "line": dict(color=BRAND_GREEN, width=1.3, dash="dash"),
        "group": "Component models",
    },
    {
        "value": "Residual",
        "label": "Residual Calibrated",
        "col": "Residual_Calibrated_Forecast_MWH",
        "line": dict(color="#9467BD", width=1.3, dash="dot"),
        "group": "Correction stages",
    },
    {
        "value": "TargetedMeta",
        "label": "Targeted Meta",
        "col": "Targeted_Meta_Adjusted_Forecast_MWH",
        "line": dict(color="#8C564B", width=1.3, dash="dot"),
        "group": "Correction stages",
    },
    {
        "value": "WarmRamp",
        "label": "Warm Ramp",
        "col": "Warm_Ramp_Adjusted_Forecast_MWH",
        "line": dict(color="#E377C2", width=1.3, dash="dot"),
        "group": "Correction stages",
    },
    {
        "value": "CloudSolar",
        "label": "Cloud / Solar",
        "col": "Cloud_Solar_Adjusted_Forecast_MWH",
        "line": dict(color="#1F77B4", width=1.3, dash="dot"),
        "group": "Correction stages",
    },
    {
        "value": "PeakRisk",
        "label": "Peak Risk",
        "col": "Peak_Risk_Adjusted_Forecast_MWH",
        "line": dict(color="#BCBD22", width=1.3, dash="dot"),
        "group": "Correction stages",
    },
    {
        "value": "RecentCorrected",
        "label": "Recent Corrected",
        "col": "Recent_Corrected_Forecast_MWH",
        "line": dict(color="#17BECF", width=1.3, dash="dot"),
        "group": "Correction stages",
    },
    {
        "value": "ScenWarmer",
        "label": "Scenario +3F",
        "col": "WeatherScenario_warmer_P50_MWH",
        "line": dict(color="#e85151", width=1.2, dash="dashdot"),
        "group": "Weather scenarios",
    },
    {
        "value": "ScenHot5",
        "label": "Scenario +5F",
        "col": "WeatherScenario_hot_stress_5f_P50_MWH",
        "line": dict(color="#c91f1f", width=1.2, dash="dashdot"),
        "group": "Weather scenarios",
    },
    {
        "value": "ScenCooler",
        "label": "Scenario -3F",
        "col": "WeatherScenario_cooler_P50_MWH",
        "line": dict(color="#6074f3", width=1.2, dash="dashdot"),
        "group": "Weather scenarios",
    },
    {
        "value": "Previous",
        "label": "Historical Forecast",
        "col": "Previous_Forecast_MWH",
    ...
    },
    {
        ...
        "col": "Prior_Run_Forecast_MWH",
        "line": dict(color="#5A5A5A", width=2.0, dash="dashdot"),
        "group": "Prior runs",
    },
]

# Toggles that are not simple lines (band fill, delta bars) but are selected the same way.
FORECAST_TOGGLE_SERIES = [
    {"value": "band", "label": "Confidence Band", "col": None, "group": "Reference"},
    {
        "value": "ForecastChange",
        "label": "Forecast Change (bars)",
        "col": "Forecast_Change_From_Prior_Run_MWH",
        "group": "Prior runs",
    },
    {
        "value": "PreviousMiss",
        "label": "Previous Miss (bars)",
        "col": "Previous_Forecast_Miss_MWH",
        "group": "Prior runs",
    },
]
...
def _merge_weather_source(
    ...
    for spec in specs.values():
        ...
        for source_col in spec["source_cols"]:
            ...
            values = (
                source_values if values is None else values.combine_first(source_values)
            )
        if values is None:
            continue
    ...
    prior = prior.dropna(subset=["__DT_KEY"]).drop_duplicates(
        subset=["__DT_KEY"], keep="last"
    )
...
def _merge_prior_weather_source(
    out: pd.DataFrame, source_df: pd.DataFrame | None
) -> pd.DataFrame:
    return _merge_weather_source(out, source_df, PRIOR_WEATHER_SERIES, "prior_col")


def _attach_forecast_run_weather(
    display_df: pd.DataFrame, forecast_weather_df: pd.DataFrame | None
) -> pd.DataFrame:
    ...
    out = _merge_weather_source(
        out, forecast_weather_df, FORECAST_WEATHER_SERIES, "forecast_col"
    )
...
def forecast_series_controls(df: pd.DataFrame):
    """Return (options, default_values) for the series selector, including only
    series whose columns exist and carry data in the loaded frame."""
    cols = set(df.columns) if df is not None else set()

    def _present(col):
        return col is None or (
            col in cols and pd.to_numeric(df[col], errors="coerce").notna().any()
        )

    options = []
    for s in FORECAST_SERIES_REGISTRY + FORECAST_TOGGLE_SERIES:
        if _present(s.get("col")):
            options.append(
                {"label": s["label"], "value": s["value"], "group": s.get("group", "")}
            )
    default = [
        v for v in DEFAULT_FORECAST_SERIES if any(o["value"] == v for o in options)
    ]
    return options, default


def _make_forecast_graph(
    df: pd.DataFrame,
    ...
    if selected_series is None:
        sel = set(DEFAULT_FORECAST_SERIES) | {
            "Raw",
            "Prophet",
            "Previous",
            "PriorRun",
        ...
        sel = set(selected_series)

    def _want(value: str) -> bool:
        return value in sel
    ...
    has_weather = bool(
        weather_variable
        and weather_variable != "none"
        and weather_variable in df.columns
    )
    forecast_weather_spec = (
        FORECAST_WEATHER_SERIES.get(weather_variable or "") if has_weather else None
    )
    forecast_weather_col = (
        forecast_weather_spec.get("forecast_col") if forecast_weather_spec else None
    )
    has_forecast_weather = bool(forecast_weather_col and _has(forecast_weather_col))
    prior_weather_spec = (
        PRIOR_WEATHER_SERIES.get(weather_variable or "") if has_weather else None
    )
    prior_weather_col = (
        prior_weather_spec.get("prior_col") if prior_weather_spec else None
    )
    ...
    # Confidence band first (filled between upper/lower) so lines draw on top.
        ...
        fig.add_trace(
            go.Scatter(
                ...
                customdata=np.column_stack(
                    [
                        pd.to_numeric(
                            df.get("Band", pd.Series(np.nan, index=df.index)),
                            errors="coerce",
                        ),
                        display_band,
                    ]
    ...
    # Delta bars (optional, off by default to reduce clutter).
    if show_change:
        ...
        fig.add_trace(
            go.Bar(
                ...
                marker_color=np.where(
                    delta.fillna(0.0) >= 0.0, BRAND_BLUE, BRAND_PURPLE
                ),
                opacity=0.24,
                yaxis=delta_axis,
    ...
    if show_miss:
        ...
        fig.add_trace(
            go.Bar(
                ...
                name="Previous Miss",
                marker_color=np.where(
                    miss.fillna(0.0) >= 0.0, BRAND_GOLD, BRAND_PURPLE
                ),
                opacity=0.32,
                yaxis=miss_axis,
    ...
    # All registry line series, drawn in declared order; the published forecast is
    # part of the registry so it can be toggled too. Reference/Actual draws early,
    # the published forecast is intentionally last among lines so it sits on top.
    line_order = [s for s in FORECAST_SERIES_REGISTRY if s["value"] != "Forecast"]
    forecast_entry = next(
        (s for s in FORECAST_SERIES_REGISTRY if s["value"] == "Forecast"), None
    )
    if forecast_entry is not None:
        line_order = line_order + [forecast_entry]
    for s in line_order:
        if _want(s["value"]) and _has(s["col"]):
            fig.add_trace(
                go.Scatter(
                    x=x_hour_ending,
                    y=pd.to_numeric(df[s["col"]], errors="coerce"),
                    mode="lines",
                    name=s["label"],
                    line=dict(**s["line"]),
    ...
    # Actual -> Forecast handoff connector only when both are shown.
    if (
        _want("Actual")
        and _want("Forecast")
        and {"DT", "Actual", "Forecast"}.issubset(df.columns)
    ):
        actual_rows = df[pd.to_numeric(df["Actual"], errors="coerce").notna()].copy()
        forecast_rows = df[
            pd.to_numeric(df["Forecast"], errors="coerce").notna()
        ].copy()
        if not actual_rows.empty and not forecast_rows.empty:
            ...
            if pd.to_datetime(first_forecast["DT"], errors="coerce") > pd.to_datetime(
                latest_actual["DT"], errors="coerce"
            ):
                fig.add_trace(
                    go.Scatter(
                        x=[
                            _hour_ending_dt(latest_actual["DT"]),
                            _hour_ending_dt(first_forecast["DT"]),
                        ],
    ...
    if has_weather:
        ...
        if has_forecast_weather:
            actual_mask = pd.to_numeric(
                df.get("Actual", pd.Series(np.nan, index=df.index)), errors="coerce"
            ).notna()
            if actual_mask.any():
        ...
        if has_forecast_weather:
            fig.add_trace(
                go.Scatter(
                    x=x_hour_ending,
                    y=pd.to_numeric(df[forecast_weather_col], errors="coerce"),
                    mode="lines",
                    name=forecast_weather_spec["label"],
                    yaxis="y2",
        ...
        if has_prior_weather:
            fig.add_trace(
                go.Scatter(
                    x=x_hour_ending,
                    y=pd.to_numeric(df[prior_weather_col], errors="coerce"),
                    mode="lines",
                    name=prior_weather_spec["label"],
                    yaxis="y2",
    ...
    yaxis2_title = (
        weather_variable if has_weather else ("Delta MWh" if has_delta_axis else "")
    )
    yaxis3 = None
    ...
    fig.update_layout(
        ...
        xaxis={
            "title": "Date / Hour Ending",
            "showgrid": True,
            "gridcolor": "rgba(0,87,184,0.10)",
        },
...
def _attach_previous_forecast_history(
    display_df: pd.DataFrame, backtest_df: pd.DataFrame
) -> pd.DataFrame:
    if (
        display_df is None
        or display_df.empty
        or backtest_df is None
    ...
    forecast_col = next(
        (c for c in forecast_candidates if c in backtest_df.columns), None
    )
    ...
    prev["Previous_Forecast_MWH"] = pd.to_numeric(
        prev["Previous_Forecast_MWH"], errors="coerce"
    )

    if "Final_Residual_MWH" in prev.columns:
        prev["Previous_Forecast_Miss_MWH"] = pd.to_numeric(
            prev["Final_Residual_MWH"], errors="coerce"
        )
    elif "Actual_MWH" in prev.columns:
        prev["Previous_Forecast_Miss_MWH"] = (
            pd.to_numeric(prev["Actual_MWH"], errors="coerce")
            - prev["Previous_Forecast_MWH"]
        )
    else:
        prev["Previous_Forecast_Miss_MWH"] = np.nan
    if "Final_AbsError_MWH" in prev.columns:
        prev["Previous_Forecast_AbsMiss_MWH"] = pd.to_numeric(
            prev["Final_AbsError_MWH"], errors="coerce"
        )
    else:
    ...
    if component_cols:
        ...
        component.rename(
            columns={c: f"__hist_{c}" for c in component_cols}, inplace=True
        )
        prev = prev.merge(component, on="DT", how="left")

    prev_keep = [
        "DT",
        "Previous_Forecast_MWH",
        "Previous_Forecast_Miss_MWH",
        "Previous_Forecast_AbsMiss_MWH",
    ]
...
def _attach_prior_run_comparison(
    ...
    if (
        previous_forecast_df is not None
        and not previous_forecast_df.empty
        and "DT" in previous_forecast_df.columns
    ):
        forecast_col = (
            "Forecast" if "Forecast" in previous_forecast_df.columns else None
        )
        if (
            forecast_col is None
            and "Final_Forecast_MWH" in previous_forecast_df.columns
        ):
        ...
        if forecast_col is not None:
            ...
            prev["Prior_Run_Forecast_MWH"] = pd.to_numeric(
                prev["Prior_Run_Forecast_MWH"], errors="coerce"
            )
            prev = prev.dropna(subset=["__DT_KEY"]).drop_duplicates(
                subset=["__DT_KEY"], keep="last"
            )
            out = out.merge(
                prev[["__DT_KEY", "Prior_Run_Forecast_MWH"]], on="__DT_KEY", how="left"
            )
            out["Forecast_Change_From_Prior_Run_MWH"] = pd.to_numeric(
                out.get("Forecast"), errors="coerce"
    ...
    if "Temperature" in out.columns and "Prior_Run_Temperature_F" in out.columns:
        out["Temperature_Change_From_Prior_Run_F"] = pd.to_numeric(
            out.get("Temperature"), errors="coerce"
...
def _make_backtest_graph(backtest_df: pd.DataFrame):
    ...
    fig.add_trace(
        go.Scatter(
            x=x_hour_ending,
            y=backtest_df["Actual_MWH"],
            mode="lines",
            name="Actual",
            line=dict(color=BRAND_RED, width=2.0),
        )
    )
    fig.add_trace(
        go.Scatter(
            x=x_hour_ending,
            y=backtest_df["Raw_Forecast_MWH"],
            mode="lines",
            name="Holdout Raw XGB+LGB",
            line=dict(color=BRAND_PURPLE, width=1.5, dash="dot"),
        )
    )
    if "Recent_Corrected_Forecast_MWH" in backtest_df.columns:
        fig.add_trace(
            go.Scatter(
                x=x_hour_ending,
                y=backtest_df["Recent_Corrected_Forecast_MWH"],
                mode="lines",
                name="Recent Corrected",
                line=dict(color=BRAND_BLUE, width=2.2),
            )
        )
    if "Prophet_Pred_MWH" in backtest_df.columns:
        fig.add_trace(
            go.Scatter(
                x=x_hour_ending,
                y=backtest_df["Prophet_Pred_MWH"],
                mode="lines",
                name="Prophet",
                line=dict(color=BRAND_GREEN, width=1.2, dash="dash"),
            )
        )
    fig.add_trace(
        go.Bar(
            x=x_hour_ending,
            y=backtest_df["Residual_MWH"],
            name="Residual",
...
def _make_error_by_hour_graph(backtest_df: pd.DataFrame):
    ...
    if "Recent_Corrected_Residual_MWH" in work.columns:
        work["DisplayResidual"] = pd.to_numeric(
            work["Recent_Corrected_Residual_MWH"], errors="coerce"
        )
    ...
    grp = work.groupby("Hour", as_index=False).agg(
        MAE=("DisplayAbsError", "mean"), Bias=("DisplayResidual", "mean")
    )
    fig.add_trace(
        go.Bar(x=grp["Hour"], y=grp["MAE"], name="MAE", marker_color=BRAND_BLUE)
    )
    fig.add_trace(
        go.Scatter(
            x=grp["Hour"],
            ...
            mode="lines+markers",
            name="Bias",
            line=dict(color=BRAND_PURPLE, width=2.0),
        )
    )
    fig.update_layout(
        title=title,
...
def _make_validation_detail_graph(backtest_df: pd.DataFrame):
    ...
    if "Final_Residual_MWH" in work.columns:
        work["DisplayResidual"] = pd.to_numeric(
            work["Final_Residual_MWH"], errors="coerce"
        )
        ...
        work["DisplayResidual"] = pd.to_numeric(
            work.get("Residual_MWH"), errors="coerce"
        )
    ...
    if "Hour" in work.columns:
        ...
        fig.add_trace(
            go.Bar(
                x=grp["Hour"], y=grp["MAE"], name="MAE by Hour", marker_color=BRAND_BLUE
            ),
            row=1,
            col=1,
            secondary_y=False,
    ...
    colors = [
        BRAND_RED if np.isfinite(v) and abs(v) >= 0.30 else BRAND_GREEN for v in acf
    ]
    fig.add_trace(
        go.Bar(
...
def _make_stage_marginal_contributions_graph(
    diagnostics_results: dict, backtest_df: pd.DataFrame
):
    contributions = (diagnostics_results or {}).get("stage_marginal_contributions")
    if (
        contributions is None
        or not isinstance(contributions, pd.DataFrame)
        or contributions.empty
    ):
        try:
            from forecasting.diagnostics.forecast_diagnostics import (
                build_stage_marginal_contributions,
            )
    ...
    if (
        ...
        fig.update_layout(
            title="Correction Stage Marginal Contributions unavailable",
            plot_bgcolor="white",
            paper_bgcolor="white",
        )
        return fig
    ...
    df_overall["Marginal_dMAE_MWH"] = pd.to_numeric(
        df_overall["Marginal_dMAE_MWH"], errors="coerce"
    )
    ...
    if df_plot.empty:
        fig.update_layout(
            title="Correction Stage Marginal Contributions empty",
            plot_bgcolor="white",
            paper_bgcolor="white",
        )
        return fig
...
def _make_production_scorecard_graph(diagnostics_results: dict):
    ...
    if scorecard is None or not isinstance(scorecard, pd.DataFrame) or scorecard.empty:
        fig.update_layout(
            title="Production Readiness Scorecard unavailable",
            plot_bgcolor="white",
            paper_bgcolor="white",
        )
        return fig
    stale = bool(
        (diagnostics_results or {})
        .get("_stale_scorecards", {})
        .get("production_readiness_scorecard", False)
    )
    work = scorecard.copy()
    ...
    work["MAE_Gate_MWH"] = gate_text.map(
        lambda gate: (
            float(gate.split("mae<=", 1)[1].split(";")[0])
            if "mae<=" in gate
            else np.nan
        )
    )
    ...
    passed = (
        work.get("Pass", pd.Series(False, index=work.index))
        .astype(str)
        .str.lower()
        .eq("true")
    )
    ...
    fig.update_layout(
        title="Official Production Readiness Scorecard - Replay First"
        + (" (stale vs current forecast)" if stale else ""),
...
def _table_data(df: pd.DataFrame):
    ...
    keep = [
        ...
        "Peak_Risk_Adjusted_Forecast_MWH",
        "Recent_Corrected_Forecast_MWH",
        "Prior_Run_Forecast_MWH",
        "Forecast_Change_From_Prior_Run_MWH",
        "P10_Forecast_MWH",
        "P50_Forecast_MWH",
        "P90_Forecast_MWH",
        "Forecast_Low_MWH",
        ...
        "Lower_Band",
        "Band",
        "Operational_Horizon_Label",
        "Production_Confidence_Label",
        "Production_Risk_Code",
        "Production_Caution_Reason",
        ...
        "Temperature",
        "Prior_Run_Temperature_F",
        "Temperature_Change_From_Prior_Run_F",
        "Temperature_DailyMax",
        "BTM_Solar_Proxy_MW",
    ...
    if "Scenario Cap" in t.columns:
        cap_flag = (
            t["Scenario Cap"].astype(str).str.lower().isin({"true", "1", "1.0", "yes"})
        )
...
def _temp_sensitivity_table_data(frame: pd.DataFrame, selected_steps: list[str] | None):
    ...
    for key, label, _delta, _color in ordered:
        if key in frame.columns:
            table[label] = pd.to_numeric(frame[key], errors="coerce").map(
...
def _scorecard_metric(
    scorecard: pd.DataFrame, test_name: str, value_col: str = "MAE_MWH"
) -> tuple[str, str] | None:
...
def _cards(
    ...
    horizon_days: int,
    diagnostics_results: dict | None = None,
):
    fut = _future_only(display_df, horizon_days)
    forecast = pd.to_numeric(fut.get("Forecast"), errors="coerce")
    raw = (
        pd.to_numeric(fut.get("Raw_Forecast_MWH"), errors="coerce")
        if "Raw_Forecast_MWH" in fut
        else pd.Series(dtype=float)
    )
    peak_dt = fut.loc[forecast.idxmax(), "DT"] if forecast.notna().any() else None
    peak_delta = (
        (forecast.max() - raw.max())
        if forecast.notna().any() and raw.notna().any()
        else np.nan
    )
    ...
    cards = [
        _metric_card(
            "Forecast Energy",
            f"{forecast.sum():,.0f} MWh" if forecast.notna().any() else "N/A",
            f"Next {horizon_days or 16} days",
        ),
        _metric_card(
            "Forecast Peak",
            f"{forecast.max():,.1f} MW" if forecast.notna().any() else "N/A",
            (
                _format_hour_ending(peak_dt, include_year=False)
        ...
        _metric_card(
            "Calibration Impact",
            f"{peak_delta:+,.1f} MW" if not pd.isna(peak_delta) else "N/A",
            "Peak vs raw model",
        ),
    ]
    if (
        display_df is not None
        and not display_df.empty
        and "Actual" in display_df.columns
    ):
        actual_rows = display_df[
            pd.to_numeric(display_df["Actual"], errors="coerce").notna()
        ].copy()
        if not actual_rows.empty and "DT" in actual_rows.columns:
            ...
            latest_source = (
                str(actual_rows.loc[latest_idx, "Load_Source"])
            ...
            if "Load_Source" in actual_rows.columns:
                five_min_hours = int(
                    actual_rows["Load_Source"]
                    .astype(str)
                    .eq("five_min_completed_hour")
                    .sum()
                )
                cards.append(
                    _metric_card(
                        "5-Min Anchored Hours",
                        f"{five_min_hours}",
                        "Completed hours appended to load history",
                    )
                )
    if not fut.empty and "DT" in fut.columns:
        ...
        cards.append(
            _metric_card(
                "First Forecast Hour",
                _format_hour_ending(first_forecast_dt, include_year=False),
                "Starts after latest actual",
            )
        )
    if "Forecast_Change_From_Prior_Run_MWH" in fut.columns:
        delta = pd.to_numeric(
            fut["Forecast_Change_From_Prior_Run_MWH"], errors="coerce"
        )
        if delta.notna().any():
            cards.append(
                _metric_card(
                    "Prior Run Delta",
                    f"{delta.abs().max():,.1f} MWh",
                    f"Mean {delta.mean():+,.1f} MWh",
                )
            )
    if "Temperature_Change_From_Prior_Run_F" in fut.columns:
        temp_delta = pd.to_numeric(
            fut["Temperature_Change_From_Prior_Run_F"], errors="coerce"
        )
        if temp_delta.notna().any():
            cards.append(
                _metric_card(
                    "Weather Delta",
                    f"{temp_delta.abs().max():,.1f} F",
                    f"Mean {temp_delta.mean():+,.1f} F",
                )
            )
    if backtest_df is not None and not backtest_df.empty:
        if "Recent_Corrected_Residual_MWH" in backtest_df.columns:
            residual = pd.to_numeric(
                backtest_df["Recent_Corrected_Residual_MWH"], errors="coerce"
            )
        ...
        mape = np.nanmean(
            np.where(actual.abs() > 1e-9, residual.abs() / actual.abs() * 100.0, np.nan)
        )
        cards.extend(
            [
        ...
        if np.isfinite(acf1):
            status = (
                "Caution: serial dependence"
                if abs(acf1) >= 0.30
                else "Low serial dependence"
            )
            cards.append(_metric_card("Residual ACF Lag 1", f"{acf1:+.2f}", status))
        if np.isfinite(dw):
            cards.append(
                _metric_card(
                    "Durbin-Watson", f"{dw:.2f}", "2.0 is roughly uncorrelated"
                )
            )
    if "Production_Caution_Flag" in fut.columns:
        caution = (
            pd.to_numeric(fut["Production_Caution_Flag"], errors="coerce")
            .fillna(0)
            .gt(0)
        )
        reasons = fut.get(
            "Production_Caution_Reason", pd.Series("", index=fut.index)
        ).astype(str)
        cards.append(
            _metric_card(
                "Caution Hours",
                f"{int(caution.sum())}",
                "Peak, weather, solar, or long-horizon",
            )
        )
        cards.append(
            _metric_card(
                "Hot-Peak Caution",
                f"{int(reasons.str.contains('hot_peak', case=False, na=False).sum())}",
                "Data-limited point accuracy",
            )
        )
    scorecard = (diagnostics_results or {}).get("production_readiness_scorecard")
    if bool(
        ...
        cards.append(
            _metric_card(
                "Scorecard Freshness",
                "Stale",
                "Rerun rolling-origin replay for current forecast",
            )
        )
    if isinstance(scorecard, pd.DataFrame) and not scorecard.empty:
        ...
        if seasonal:
            cards.append(
                _metric_card(
                    "Official Replay MAE",
                    seasonal[1],
                    f"Seasonal rolling origins: {seasonal[0]}",
                )
            )
        if day1:
        ...
        if hot:
            cards.append(
                _metric_card("Hot Peak Gate", hot[1], f"Data-limited: {hot[0]}")
            )
        if peak:
            cards.append(
                _metric_card("Peak Window Gate", peak[1], f"Data-limited: {peak[0]}")
            )
    return cards
...
def _make_daily_peak_miss_graph(diagnostics_results: dict):
    ...
    if isinstance(daily, pd.DataFrame) and not daily.empty and "Stage" in daily.columns:
        preferred = daily[
            daily["Stage"].isin(
                ["final_corrected_production", "recent_corrected_simulation"]
            )
        ]
    ...
    if daily is None or not isinstance(daily, pd.DataFrame) or daily.empty:
        fig.update_layout(
            title="Daily Peak Miss Diagnostics unavailable",
            plot_bgcolor="white",
            paper_bgcolor="white",
        )
        return fig
...
def _make_segment_error_graph(diagnostics_results: dict):
    ...
    if seg is None or not isinstance(seg, pd.DataFrame) or seg.empty:
        fig.update_layout(
            title="Segment Error Diagnostics unavailable",
            plot_bgcolor="white",
            paper_bgcolor="white",
        )
        return fig
    ...
    columns = [
        {
            "name": (
                "Hour Ending"
                if c == "DT"
                else ("Actual Peak HE" if c == "Actual_Peak_DT" else c)
            ),
            "id": c,
        }
        for c in t.columns
...
def create_dashboard_app(
    ...
    previous_forecast_snapshot = (future_results or {}).get(
        "previous_forecast_snapshot", pd.DataFrame()
    )
    previous_weather_snapshot = (future_results or {}).get(
        "previous_weather_snapshot", pd.DataFrame()
    )
    forecast_weather_snapshot = (future_results or {}).get(
        "current_weather_snapshot", pd.DataFrame()
    )
    if (
        not isinstance(forecast_weather_snapshot, pd.DataFrame)
        or forecast_weather_snapshot.empty
    ) and isinstance(diagnostics_results, dict):
        forecast_weather_snapshot = diagnostics_results.get(
            "forecast_weather_used", pd.DataFrame()
        )
    hist_df = (
        historical_fit_df.copy() if historical_fit_df is not None else pd.DataFrame()
    )
    if not hist_df.empty and "DT" in hist_df.columns:
        hist_df["DT"] = _coerce_dt(hist_df["DT"])
    backtest_df = (
        backtest_results.copy() if backtest_results is not None else pd.DataFrame()
    )
    ...
    display_df = _attach_prior_run_comparison(
        display_df, previous_forecast_snapshot, previous_weather_snapshot
    )

    # Dates for the v11.6-style date picker.
    min_date = (
        pd.to_datetime(display_df["DT"].min(), errors="coerce").date()
        if not display_df.empty
        else None
    )
    max_date = (
        pd.to_datetime(display_df["DT"].max(), errors="coerce").date()
        if not display_df.empty
        else None
    )
    if not display_df.empty and display_df.get("Forecast").notna().any():
        default_start_date = pd.to_datetime(
            display_df.loc[display_df["Forecast"].notna(), "DT"].min(), errors="coerce"
        ).date()
    else:
        default_start_date = (
            pd.to_datetime(display_df["DT"].max(), errors="coerce").date()
    ...
    available_days = (
        sorted(
            pd.to_datetime(display_df["DT"], errors="coerce")
            .dt.date.dropna()
            .unique()
            .tolist()
        )
    ...
    series_options, series_default = forecast_series_controls(display_df)
    ...
    def _temp_sensitivity_history(
        hist: pd.DataFrame, display: pd.DataFrame
    ) -> pd.DataFrame:
        frames = []
        if (
            hist is not None
            and not hist.empty
            and {"DT", "MWH", "Temperature"}.issubset(hist.columns)
        ):
            frames.append(hist[["DT", "MWH", "Temperature"]].copy())
        if (
            display is not None
            and not display.empty
            and {"DT", "Actual", "Temperature"}.issubset(display.columns)
        ):
            display_hist = display.loc[
                display["Actual"].notna(), ["DT", "Actual", "Temperature"]
            ].copy()
        ...
        work = work.dropna(subset=["DT", "MWH", "Temperature"]).drop_duplicates(
            subset=["DT"], keep="first"
        )
    ...
    def _make_temp_sensitivity_frame(
        display: pd.DataFrame, hist: pd.DataFrame
    ) -> pd.DataFrame:
        if (
            display is None
            or display.empty
            or not {"DT", "Forecast"}.issubset(display.columns)
        ):
            return pd.DataFrame()
        ...
        frame["Temp_Sensitivity_MWH_Per_F"] = (
            frame["DT"].dt.hour.map(slopes).fillna(0.0)
        )
        for key, _label, delta, _color in TEMP_SENSITIVITY_STEPS:
            frame[key] = frame["Forecast"] + (
                frame["Temp_Sensitivity_MWH_Per_F"] * delta
            )
        return frame
    ...
    def _make_temp_sensitivity_graph(
        ...
        if frame is None or frame.empty:
            fig.update_layout(
                title="Temperature Sensitivity unavailable",
                plot_bgcolor="white",
                paper_bgcolor="white",
            )
            return fig
    ...
    def _make_comparable_days_graph(
        display: pd.DataFrame, hist: pd.DataFrame, start_d: date_type | None
    ) -> go.Figure:
        ...
        if (
            ...
            fig.update_layout(
                title="Comparable Days unavailable",
                plot_bgcolor="white",
                paper_bgcolor="white",
            )
            return fig
        ...
        fut_day = display[
            (display["DT"] >= day_start) & (display["DT"] < day_end)
        ].copy()
        fut_day["Temperature"] = pd.to_numeric(
            fut_day.get("Temperature"), errors="coerce"
        )
        target_max = (
            float(fut_day["Temperature"].max()) if "Temperature" in fut_day else np.nan
        )
        ...
        hist_work["Temperature"] = pd.to_numeric(
            hist_work.get("Temperature"), errors="coerce"
        )
        ...
        if not np.isfinite(target_max) or daily.empty:
            fig.update_layout(
                title="Comparable Days (No target temperature)",
                plot_bgcolor="white",
                paper_bgcolor="white",
            )
            return fig
        ...
        candidates = (
            daily.sort_values(["TempDiff", "Date"], ascending=[True, False])
            .head(3)["Date"]
            .tolist()
        )
    ...
    @app.callback(
        ...
        Input({"type": "series-group", "group": ALL}, "value"),
    ...
    def update_dashboard(
        ...
        history_hours = (
            72  # v11 uses a start-date anchor; keep a small history context.
        )
        start_d = _parse_date(start_date)
        df = _window(display_df, start_d, int(horizon_days or 16), history_hours)
        cards = _cards(
            display_df, backtest_df, int(horizon_days or 16), diagnostics_results
        )
        ...
        if not diagnostics_mode and columns:
            ...
            data = [
                {k: row.get(k, "") for k in [c["id"] for c in columns]} for row in data
            ]
        ...
        if model_type == "temp_sens":
            ...
            show_weather = (
                True if (graph_options and "show_weather" in graph_options) else False
            )
            highlight_max = (
                True if (graph_options and "highlight_max" in graph_options) else False
            )
            aux_fig = _make_temp_sensitivity_graph(
                sensitivity, temperature_steps, show_weather, highlight_max
            )
            ...
            # series_selected arrives as one value-list per group (pattern-matching
            # inputs); flatten into a single list of selected series values.
            if series_selected and isinstance(series_selected[0], (list, tuple)):
                series_flat = [v for grp in series_selected for v in (grp or [])]
            else:
                series_flat = list(series_selected or [])
            main_fig = _make_forecast_graph(df, wvar, series_flat)

        if isinstance(
            (diagnostics_results or {}).get("production_readiness_scorecard"),
            pd.DataFrame,
        ):
        ...
        stats_marginal = _make_stage_marginal_contributions_graph(
            diagnostics_results, backtest_df
        )
