...
FORECAST_SERIES_REGISTRY = [
    ...
    {"value": "XGB",             "label": "XGB",                "col": "XGB_Pred_MWH",                        "line": dict(color="#7B2CBF",    width=1.4, dash="dot"),    "group": "Component models"},
    {"value": "LGB",             "label": "LGB",                "col": "LGB_Pred_MWH",                        "line": dict(color="#17A2B8",    width=1.4, dash="dot"),    "group": "Component models"},
    {"value": "CatBoost",        "label": "CatBoost",           "col": "CatBoost_Pred_MWH",                   "line": dict(color="#FF7F0E",    width=1.4, dash="dot"),    "group": "Component models"},
    {"value": "Prophet",         "label": "Prophet",            "col": "Prophet_Pred_MWH",                    "line": dict(color=BRAND_GREEN,  width=1.3, dash="dash"),   "group": "Component models"},
    {"value": "Residual",        "label": "Residual Calibrated","col": "Residual_Calibrated_Forecast_MWH",    "line": dict(color="#9467BD",    width=1.3, dash="dot"),    "group": "Correction stages"},
    {"value": "TargetedMeta",    "label": "Targeted Meta",      "col": "Targeted_Meta_Adjusted_Forecast_MWH", "line": dict(color="#8C564B",    width=1.3, dash="dot"),    "group": "Correction stages"},
    {"value": "WarmRamp",        "label": "Warm Ramp",          "col": "Warm_Ramp_Adjusted_Forecast_MWH",     "line": dict(color="#E377C2",    width=1.3, dash="dot"),    "group": "Correction stages"},
    {"value": "CloudSolar",      "label": "Cloud / Solar",      "col": "Cloud_Solar_Adjusted_Forecast_MWH",   "line": dict(color="#1F77B4",    width=1.3, dash="dot"),    "group": "Correction stages"},
    {"value": "PeakRisk",        "label": "Peak Risk",          "col": "Peak_Risk_Adjusted_Forecast_MWH",     "line": dict(color="#BCBD22",    width=1.3, dash="dot"),    "group": "Correction stages"},
    {"value": "RecentCorrected", "label": "Recent Corrected",   "col": "Recent_Corrected_Forecast_MWH",       "line": dict(color="#17BECF",    width=1.3, dash="dot"),    "group": "Correction stages"},
...
FORECAST_TOGGLE_SERIES = [
    ...
    {"value": "ForecastChange", "label": "Forecast Change (bars)", "col": "Forecast_Change_From_Prior_Run_MWH",  "group": "Prior runs"},
...
def forecast_series_controls(df: pd.DataFrame):
    ...
    def _present(col):
        return col is None or (col in cols and pd.to_numeric(df[col], errors="coerce").notna().any())

    options = []
    for s in FORECAST_SERIES_REGISTRY + FORECAST_TOGGLE_SERIES:
        if _present(s.get("col")):
            options.append({"label": s["label"], "value": s["value"], "group": s.get("group", "")})
    default = [v for v in DEFAULT_FORECAST_SERIES if any(o["value"] == v for o in options)]
...
def _make_forecast_graph(df: pd.DataFrame, weather_variable: str | None, selected_series: list[str] | None = None):
    ...
    if selected_series is None:
        sel = set(DEFAULT_FORECAST_SERIES) | {"Raw", "Prophet", "Previous", "PriorRun", "ForecastChange", "PreviousMiss"}
    else:
    ...
    forecast_entry = next((s for s in FORECAST_SERIES_REGISTRY if s["value"] == "Forecast"), None)
    ...
    if _want("Actual") and _want("Forecast") and {"DT", "Actual", "Forecast"}.issubset(df.columns):
