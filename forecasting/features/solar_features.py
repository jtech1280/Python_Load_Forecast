...
def _solar_season_factor(dt: pd.Series) -> pd.Series:
    ...
    return pd.Series(
        0.825 + 0.175 * np.cos(2.0 * np.pi * (doy - 172.0) / 365.25), index=dt.index
    ).clip(0.60, 1.05)
...
def add_solar_features(
    df: pd.DataFrame, btm_monthly_df: pd.DataFrame, solar_df: pd.DataFrame | None = None
) -> pd.DataFrame:
    out = df.copy()
    ...
    out["Capacity_Ratio_To_Current"] = (
        out["Capacity_Ratio_To_Current"].ffill().bfill().fillna(0.0)
    )
    ...
    out["ClearSky_GHI_Proxy_Wm2"] = (
        1000.0 * out["Solar_Hour_Shape"] * out["Solar_Season_Factor"]
    ).clip(lower=0.0)
    denom = out["ClearSky_GHI_Proxy_Wm2"].replace(0.0, np.nan)
    out["ClearSky_Index"] = (
        (out["Solar_Irradiance"] / denom)
    ...
    out["BTM_Solar_Proxy_MW"] = (
        explicit_solar.combine_first(weather_solar_proxy).fillna(0.0).clip(lower=0.0)
    )
    ...
    out["BTM_Solar_Loss_From_ClearSky_MW"] = (
        out["BTM_ClearSky_Proxy_MW"] - out["BTM_Solar_Cloud_Adjusted_MW"]
    ).clip(lower=0.0)

    cloud = pd.to_numeric(
        out.get("CloudCover_Norm", pd.Series(0.0, index=out.index)), errors="coerce"
    ).fillna(0.0)
    ...
    out["Daily_BTM_ClearSky_Max_MW"] = out.groupby("Date")[
    ...
    out["BTM_Evening_Ramp_Impact"] = (
        out["BTM_Solar_Proxy_MW"] * out["Solar_Evening_Ramp_Flag"]
    )
    out["Midday_Overcast_Solar_Loss_MW"] = (
        out["BTM_Solar_Loss_From_ClearSky_MW"]
        * out["Solar_Midday_Flag"]
        * (cloud >= 0.60).astype(int)
    )
