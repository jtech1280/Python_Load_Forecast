...
class DashboardAppHelperTests(unittest.TestCase):
    ...
    def test_display_confidence_band_falls_back_to_upper_lower_and_caps_display_width(
        self,
    ):
        df = pd.DataFrame(
            {
    ...
    def test_make_stage_marginal_contributions_graph_renders_correctly(self):
        ...
        fig = dashboard_app._make_stage_marginal_contributions_graph(
    ...
    def test_make_forecast_graph_respects_selected_series_and_handoff(self):
        ...
        fig = dashboard_app._make_forecast_graph(
            df, weather_variable="none", selected_series=["Actual", "Forecast"]
        )
        names = [trace.name for trace in fig.data]

        self.assertEqual(
            names, ["Actual", "Published Forecast", "Actual-Forecast Handoff"]
        )
        self.assertEqual(
            pd.Timestamp(fig.data[0].x[0]), pd.Timestamp("2026-01-01 01:00")
        )
    ...
    def test_make_forecast_graph_adds_prior_run_weather_for_selected_weather(self):
        ...
        prior_trace = next(
            trace for trace in fig.data if trace.name == "Prior Run Cloud"
        )
    ...
    def test_make_forecast_graph_adds_actual_and_forecast_temperature_lines(self):
        ...
        actual_temp = next(
            trace for trace in fig.data if trace.name == "Actual Temperature"
        )
        forecast_temp = next(
            trace for trace in fig.data if trace.name == "Forecast Temperature"
        )
    ...
    def test_attach_previous_forecast_history_uses_last_duplicate_and_residual_columns(
        self,
    ):
        dt = pd.date_range("2026-01-01 00:00", periods=2, freq="h")
    ...
    def test_attach_forecast_run_weather_adds_current_forecast_weather_columns(self):
        ...
        display = pd.DataFrame(
            {"DT": dt, "Actual": [100.0, 101.0], "Temperature": [70.0, 80.0]}
        )
    ...
    def test_attach_prior_run_comparison_adds_forecast_and_weather_deltas(self):
        ...
        previous_weather = pd.DataFrame(
            {"DT": dt, "TempF": [69.0, 77.0], "HumidityPct": [45.0, 65.0]}
        )

        out = dashboard_app._attach_prior_run_comparison(
            display, previous_forecast, previous_weather
        )

        self.assertEqual(out["Prior_Run_Forecast_MWH"].tolist(), [100.0, 125.0])
        self.assertEqual(
            out["Forecast_Change_From_Prior_Run_MWH"].tolist(), [10.0, -5.0]
        )
        self.assertEqual(out["Prior_Run_Temperature_F"].tolist(), [69.0, 77.0])
        self.assertEqual(
            out["Temperature_Change_From_Prior_Run_F"].tolist(), [1.0, 3.0]
        )
    ...
    def test_temp_sensitivity_table_data_defaults_to_baseline(self):
        ...
        rows, columns = dashboard_app._temp_sensitivity_table_data(
            frame, selected_steps=None
        )

        self.assertEqual(
            [column["id"] for column in columns], ["Date", "Time", "Baseline"]
        )
        self.assertEqual(
            rows[0], {"Date": "2026-01-01", "Time": "14:00", "Baseline": "125.1"}
        )
    ...
    def test_residual_validation_prefers_final_residual_and_computes_statistics(self):
        ...
        self.assertAlmostEqual(
            dashboard_app._durbin_watson(pd.Series([1.0, 2.0, 3.0])), 2.0 / 14.0
        )
    ...
    def test_create_dashboard_app_constructs_layout_with_series_controls(self):
        ...
        self.assertTrue(
            any(
                isinstance(item, dict) and item.get("type") == "series-group"
                for item in ids
            )
        )
    ...
    def test_legacy_app_does_not_render_empty_series_message(self):
        dt = pd.date_range("2026-01-01 00:00", periods=2, freq="h")
        display = pd.DataFrame(
            {"DT": dt, "Actual": [np.nan, np.nan], "Forecast": [100.0, 101.0]}
        )
    ...
    def test_run_from_outputs_uses_series_dashboard_app(self):
        self.assertIs(
            run_from_outputs.create_dashboard_app, dashboard_app.create_dashboard_app
        )

    def test_legacy_app_module_uses_series_dashboard_app(self):
        self.assertIs(
            legacy_dashboard_app.create_dashboard_app,
